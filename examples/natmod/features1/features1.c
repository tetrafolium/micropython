...
uint16_t *const table_ptr16a[] = {&data16[0], &data16[1], &data16[2],
                                  &data16[3]};
...
STATIC mp_obj_t fibonacci(mp_obj_t x_in) {
  ...
  if (x < 0) {
    mp_raise_ValueError(
        MP_ERROR_TEXT("can't compute negative Fibonacci number"));
  }
...
STATIC mp_obj_t access(size_t n_args, const mp_obj_t *args) {
  if (n_args == 0) {
    ...
    mp_obj_list_t *lst =
        MP_OBJ_TO_PTR(mp_obj_new_list(MP_ARRAY_SIZE(data16), NULL));
...
STATIC mp_obj_t make_array(void) {
  ...
  return mp_obj_new_bytearray_by_ref(
      sizeof(uint16_t) * MP_ARRAY_SIZE(table_ptr16b), ptr);
}
