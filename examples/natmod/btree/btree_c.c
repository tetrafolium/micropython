...
void *memset(void *s, int c, size_t n) { return mp_fun_table.memset_(s, c, n); }
#endif
...
ssize_t mp_stream_posix_write(void *stream, const void *buf, size_t len) {
  ...
  mp_uint_t out_sz =
      stream_p->write(MP_OBJ_FROM_PTR(stream), buf, len, &native_errno);
...
ssize_t mp_stream_posix_read(void *stream, void *buf, size_t len) {
  ...
  const mp_stream_p_t *stream_p = o->type->protocol;
  mp_uint_t out_sz =
      stream_p->read(MP_OBJ_FROM_PTR(stream), buf, len, &native_errno);
  if (out_sz == MP_STREAM_ERROR) {
    return -1;
  } else {
    return out_sz;
...
off_t mp_stream_posix_lseek(void *stream, off_t offset, int whence) {
  ...
  mp_uint_t res = stream_p->ioctl(MP_OBJ_FROM_PTR(stream), MP_STREAM_SEEK,
                                  (mp_uint_t)(uintptr_t)&seek_s, &native_errno);
...
int mp_stream_posix_fsync(void *stream) {
  mp_obj_base_t *o = stream;
  const mp_stream_p_t *stream_p = o->type->protocol;
  mp_uint_t res = stream_p->ioctl(MP_OBJ_FROM_PTR(stream), MP_STREAM_FLUSH, 0,
                                  &native_errno);
  if (res == MP_STREAM_ERROR) {
    return -1;
  }
...
STATIC mp_obj_t btree_open(size_t n_args, const mp_obj_t *args) {
  ...
  DB *db =
      __bt_open(MP_OBJ_TO_PTR(args[0]), &btree_stream_fvtable, &openinfo, 0);
  if (db == NULL) {
...
mp_obj_t mpy_init(mp_obj_fun_bc_t *self, size_t n_args, size_t n_kw,
  ...
  btree_locals_dict_table[2] = (mp_map_elem_t){MP_OBJ_NEW_QSTR(MP_QSTR_get),
                                               MP_OBJ_FROM_PTR(&btree_get_obj)};
  btree_locals_dict_table[3] = (mp_map_elem_t){MP_OBJ_NEW_QSTR(MP_QSTR_put),
                                               MP_OBJ_FROM_PTR(&btree_put_obj)};
  btree_locals_dict_table[4] = (mp_map_elem_t){MP_OBJ_NEW_QSTR(MP_QSTR_seq),
                                               MP_OBJ_FROM_PTR(&btree_seq_obj)};
