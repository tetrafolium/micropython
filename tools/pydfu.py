...
def clr_status():
    ...
    __dev.ctrl_transfer(0x21, __DFU_CLRSTATUS, 0, __DFU_INTERFACE, None,
                        __TIMEOUT)


def get_status():
    """Get the status of the last operation."""
    stat = __dev.ctrl_transfer(0xA1, __DFU_GETSTATUS, 0, __DFU_INTERFACE, 6,
                               20000)
...
def check_status(stage, expected):
    status = get_status()
    if status != expected:
        raise SystemExit("DFU: %s failed (%s)" %
...
def mass_erase():
    ...
    __dev.ctrl_transfer(0x21, __DFU_DNLOAD, 0, __DFU_INTERFACE, "\x41",
                        __TIMEOUT)
    ...
    while xfer_bytes < xfer_total:
        if __verbose and xfer_count % 512 == 0:
            print("Addr 0x%x %dKBs/%dKBs..." %
                  (xfer_base + xfer_bytes, xfer_bytes // 1024,
                   xfer_total // 1024))
...
def exit_dfu():
    ...
    __dev.ctrl_transfer(0x21, __DFU_DNLOAD, 0, __DFU_INTERFACE, None,
                        __TIMEOUT)
...
def read_dfu_file(filename):
    ...
    for target_idx in range(dfu_prefix["targets"]):
        ...
        img_prefix, data = consume(
            "<6sBI255s2I", data, "signature altsetting named name "
            "size elements")
...
def get_dfu_devices(*args, **kwargs):
    ...
    return list(
        usb.core.find(*args, find_all=True, custom_match=FilterDFU(),
                      **kwargs))
...
def list_dfu_devices(*args, **kwargs):
    ...
    for device in devices:
        print("Bus {} Device {:03d}: ID {:04x}:{:04x}".format(
            device.bus, device.address, device.idVendor, device.idProduct))
        layout = get_memory_layout(device)
        print("Memory Layout")
        for entry in layout:
            print("    0x{:x} {:2d} pages of {:3d}K bytes".format(
            ...
            if not mass_erase_used:
                for segment in mem_layout:
                    if addr >= segment["addr"] and addr <= segment["last_addr"]:
                        ...
                        break
            write_memory(addr, data[:write_size], progress, elem_addr,
                         elem_size)
...
def cli_progress(addr, offset, size):
    ...
    print(
        "\r0x{:08x} {:7d} [{}{}] {:3d}% ".format(addr, size, "=" * done,
                                                 " " * (width - done),
                                                 offset * 100 // size),
        end="",
    )
