...
static const unsigned short aFATEntries[2][NVMEM_RM_FILEID + 1] =
    /*  address 	*/ {{0x50, 0x1f0, 0x390, 0x1390, 0x2390, 0x4390, 0x6390,
                             0x63a0, 0x63b0, 0x63f0, 0x6430, 0x6830},
                            /*  length	*/ {0x1a0, 0x1a0, 0x1000, 0x1000,
                                            0x2000, 0x2000, 0x10, 0x10, 0x40,
                                            0x40, 0x400, 0x200}};
/* 0. NVS */
/* 1. NVS Shadow */
/* 2. Wireless Conf */
...
static void CC3000_UsynchCallback(long lEventType, char *data,
...
static int initDriver(unsigned short cRequestPatch) {
  ...
  wlan_init(CC3000_UsynchCallback, sendWLFWPatch, sendDriverPatch,
            sendBootLoaderPatch, ReadWlanInterruptPin, SpiResumeSpi,
...
fat_read_content(unsigned char *is_allocated, unsigned char *is_valid,
                 unsigned char *write_protected, unsigned short *file_address,
  ...
  for (index = 0; index <= NVMEM_RM_FILEID; index++) {
    ...
    *file_address++ = ((*(fatTablePtr + 1) << 8) | (*fatTablePtr)) &
                      (BIT4 | BIT5 | BIT6 | BIT7);
    *file_length++ = ((*(fatTablePtr + 3) << 8) | (*(fatTablePtr + 2))) &
                     (BIT4 | BIT5 | BIT6 | BIT7);
...
void patch_prog_start() {
  ...
  while (return_status) {
    ...
    for (index = 0; index < 4; index++) {
      return_status |= nvmem_write(NVMEM_RM_FILEID, 32, 32 * index,
                                   (pRMParams + 32 * index));
    }
  }
  ...
  while (ucStatus_FW) {
    ...
    // different Service Packs.
    ucStatus_FW =
        nvmem_write_patch(NVMEM_WLAN_FW_SP_FILEID, fw_length, fw_patch);
  }
