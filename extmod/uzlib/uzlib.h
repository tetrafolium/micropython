...
struct uzlib_uncomp {
  ...
  /* Pointer to the next byte past the input buffer (source_limit = source +
   * len) */
...
void TINFCC uzlib_uncompress_init(TINF_DATA *d, void *dict,
...
void TINFCC uzlib_compress(struct uzlib_comp *c, const uint8_t *src,
                           unsigned slen);

/* Checksum API */

/* prev_sum is previous value for incremental computation, 1 initially */
uint32_t TINFCC uzlib_adler32(const void *data, unsigned int length,
                              uint32_t prev_sum);
/* crc is previous value for incremental computation, 0xffffffff initially */
uint32_t TINFCC uzlib_crc32(const void *data, unsigned int length,
                            uint32_t crc);
