...
STATIC mp_uint_t stest_read(mp_obj_t o_in, void *buf, mp_uint_t size,
                            int *errcode) {
  mp_obj_streamtest_t *o = MP_OBJ_TO_PTR(o_in);
  if (o->pos < o->len) {
    if (size > o->len - o->pos) {
...
STATIC mp_uint_t stest_write(mp_obj_t o_in, const void *buf, mp_uint_t size,
                             int *errcode) {
  mp_obj_streamtest_t *o = MP_OBJ_TO_PTR(o_in);
  (void)buf;
  (void)size;
...
STATIC mp_uint_t stest_ioctl(mp_obj_t o_in, mp_uint_t request, uintptr_t arg,
                             int *errcode) {
  mp_obj_streamtest_t *o = MP_OBJ_TO_PTR(o_in);
  (void)arg;
  (void)request;
...
STATIC mp_uint_t stest_read2(mp_obj_t o_in, void *buf, mp_uint_t size,
                             int *errcode) {
  (void)o_in;
  (void)buf;
  (void)size;
...
STATIC const mp_obj_str_t str_no_hash_obj = {
    {&mp_type_str}, 0, 10, (const byte *)"0123456789"};
STATIC const mp_obj_str_t bytes_no_hash_obj = {
    {&mp_type_bytes}, 0, 10, (const byte *)"0123456789"};
  ...
  for (size_t i = 0; i < nops; ++i) {
    ...
    if (mp_pairheap_is_empty(pairheap_lt, heap)) {
      ...
      mp_printf(&mp_plat_print, " %d",
                mp_pairheap_peek(pairheap_lt, heap) - &node[0]);
      ;
    }
  }
  printf("\npop all:");
...
STATIC mp_obj_t extra_coverage(void) {
  // mp_printf (used by ports that don't have a native printf)
  {
    ...
    mp_printf(&mp_plat_print, "%.2s %.3s '%4.4s' '%5.5q' '%.3q'\n", "abc",
              "abc", "abc", MP_QSTR_True,
              MP_QSTR_True);                        // fixed string precision
  ...
  // repl autocomplete
  {
    ...
    mp_store_global(MP_QSTR_sys, mp_import_name(MP_QSTR_sys, mp_const_none,
  ...
  // attrtuple
  {
    ...
    static const mp_obj_t items[] = {MP_OBJ_NEW_SMALL_INT(1),
                                     MP_OBJ_NEW_SMALL_INT(2),
                                     MP_OBJ_NEW_SMALL_INT(3)};
    mp_obj_print_helper(&mp_plat_print, mp_obj_new_attrtuple(fields, 3, items),
                        PRINT_REPR);
    mp_printf(&mp_plat_print, "\n");
  }
  ...
  // bytearray
  {
    ...
    mp_buffer_info_t bufinfo;
    mp_get_buffer_raise(mp_obj_new_bytearray(4, "data"), &bufinfo,
                        MP_BUFFER_RW);
  ...
  // runtime utils
  {
    ...
    mp_call_function_1_protected(MP_OBJ_FROM_PTR(&mp_builtin_abs_obj),
                                 MP_OBJ_NEW_SMALL_INT(1));
    // call mp_call_function_1_protected with invalid args
    mp_call_function_1_protected(MP_OBJ_FROM_PTR(&mp_builtin_abs_obj),
                                 mp_obj_new_str("abc", 3));

    // call mp_call_function_2_protected
    mp_call_function_2_protected(MP_OBJ_FROM_PTR(&mp_builtin_divmod_obj),
                                 MP_OBJ_NEW_SMALL_INT(1),
                                 MP_OBJ_NEW_SMALL_INT(1));
    ...
    // mp_obj_int_get_uint_checked with negative small-int (should raise
    // exception)
    nlr_buf_t nlr;
    if (nlr_push(&nlr) == 0) {
    ...
    // mp_obj_int_get_uint_checked with negative big-int (should raise
    // exception)
    if (nlr_push(&nlr) == 0) {
    ...
    // call mp_obj_new_exception_args (it's a part of the public C API and not
    // used in the core)
    mp_obj_print_exception(&mp_plat_print, mp_obj_new_exception_args(
                                               &mp_type_ValueError, 0, NULL));
  }
  ...
  // VM
  {
    ...
    // call mp_execute_bytecode with invalide bytecode (should raise
    // NotImplementedError)
    ...
    mp_printf(&mp_plat_print, "%d %d\n", ret,
              mp_obj_get_type(code_state->state[0]) ==
  ...
  // scheduler
  {
    ...
    for (int i = 0; i < 5; ++i) {
      mp_printf(&mp_plat_print, "sched(%d)=%d\n", i,
                mp_sched_schedule(MP_OBJ_FROM_PTR(&mp_builtin_print_obj),
    ...
    // setting the keyboard interrupt (twice) and cancelling it during
    // mp_handle_pending
    mp_keyboard_interrupt();
    mp_keyboard_interrupt();
    mp_handle_pending(false);

    // setting keyboard interrupt and a pending event (intr should be handled
    // first)
    mp_sched_schedule(MP_OBJ_FROM_PTR(&mp_builtin_print_obj),
                      MP_OBJ_NEW_SMALL_INT(10));
    mp_keyboard_interrupt();
    if (nlr_push(&nlr) == 0) {
      mp_handle_pending(true);
      nlr_pop();
  ...
  // mp_obj_is_type and derivatives
  {
    ...
    mp_printf(&mp_plat_print, "%d %d\n", mp_obj_is_bool(mp_const_true),
              mp_obj_is_bool(mp_const_false));
    mp_printf(&mp_plat_print, "%d %d\n",
              mp_obj_is_bool(MP_OBJ_NEW_SMALL_INT(1)),
              mp_obj_is_bool(mp_const_none));

    // mp_obj_is_integer accepts ints and booleans
    mp_printf(&mp_plat_print, "%d %d\n",
              mp_obj_is_integer(MP_OBJ_NEW_SMALL_INT(1)),
              mp_obj_is_integer(mp_obj_new_int_from_ll(1)));
    mp_printf(&mp_plat_print, "%d %d\n", mp_obj_is_integer(mp_const_true),
              mp_obj_is_integer(mp_const_false));
    mp_printf(&mp_plat_print, "%d %d\n",
              mp_obj_is_integer(mp_obj_new_str("1", 1)),
              mp_obj_is_integer(mp_const_none));

    // mp_obj_is_int accepts small int and object ints
    mp_printf(&mp_plat_print, "%d %d\n", mp_obj_is_int(MP_OBJ_NEW_SMALL_INT(1)),
              mp_obj_is_int(mp_obj_new_int_from_ll(1)));
  }
  ...
  mp_obj_t items[] = {(mp_obj_t)&str_no_hash_obj, (mp_obj_t)&bytes_no_hash_obj,
                      MP_OBJ_FROM_PTR(s), MP_OBJ_FROM_PTR(s2)};
