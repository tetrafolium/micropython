...
static int8_t SCSI_StartStopUnit(USBD_HandleTypeDef *pdev, uint8_t lun,
                                 uint8_t *params);
static int8_t SCSI_AllowMediumRemoval(USBD_HandleTypeDef *pdev, uint8_t lun,
                                      uint8_t *params);
...
static int8_t SCSI_ModeSense10(USBD_HandleTypeDef *pdev, uint8_t lun,
                               uint8_t *params);
static int8_t SCSI_SynchronizeCache(USBD_HandleTypeDef *pdev, uint8_t lun,
                                    uint8_t *params);
...
static int8_t SCSI_TestUnitReady(USBD_HandleTypeDef *pdev, uint8_t lun,
  ...
  USBD_MSC_BOT_HandleTypeDef *hmsc =
      &((usbd_cdc_msc_hid_state_t *)pdev->pClassData)->MSC_BOT_ClassData;
...
static int8_t SCSI_ReadCapacity10(USBD_HandleTypeDef *pdev, uint8_t lun,
  ...
  if (hmsc->bdev_ops->GetCapacity(lun, &hmsc->scsi_blk_nbr[lun],
                                  &hmsc->scsi_blk_size[lun]) != 0) {
    SCSI_SenseCode(pdev, lun, NOT_READY, MEDIUM_NOT_PRESENT);
    return -1;
  } else {
...
static int8_t SCSI_StartStopUnit(USBD_HandleTypeDef *pdev, uint8_t lun,
  ...
  // On Mac OS X, when the device is ejected a SCSI_START_STOP_UNIT command is
  // sent. Bit 0 of params[4] is the START bit. If we get a stop, we must really
  // stop the device so that the Mac does not automatically remount it.
...
static int8_t SCSI_Read10(USBD_HandleTypeDef *pdev, uint8_t lun,
  ...
  if (hmsc->bot_state == USBD_BOT_IDLE) /* Idle */
  {
    ...
    hmsc->scsi_blk_addr_in_blks =
        (params[2] << 24) | (params[3] << 16) | (params[4] << 8) | params[5];

    hmsc->scsi_blk_len = (params[7] << 8) | params[8];

    if (SCSI_CheckAddressRange(pdev, lun, hmsc->scsi_blk_addr_in_blks,
                               hmsc->scsi_blk_len) < 0) {
      return -1; /* error */
    }
...
static int8_t SCSI_Write10(USBD_HandleTypeDef *pdev, uint8_t lun,
  ...
  if (hmsc->bot_state == USBD_BOT_IDLE) /* Idle */
  {
    ...
    USBD_LL_PrepareReceive(pdev, MSC_OUT_EP, hmsc->bot_data,
...
static int8_t SCSI_ProcessRead(USBD_HandleTypeDef *pdev, uint8_t lun) {
  ...
  if (hmsc->bdev_ops->Read(lun, hmsc->bot_data, hmsc->scsi_blk_addr_in_blks,
...
static int8_t SCSI_ProcessWrite(USBD_HandleTypeDef *pdev, uint8_t lun) {
  ...
  if (hmsc->bdev_ops->Write(lun, hmsc->bot_data, hmsc->scsi_blk_addr_in_blks,
                            len / hmsc->scsi_blk_size[lun]) < 0) {
