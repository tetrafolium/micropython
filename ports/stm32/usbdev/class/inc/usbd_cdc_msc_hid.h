...
#define MAX_TEMPLATE_CONFIG_DESC_SIZE                                          \
  (9 + (23) + MICROPY_HW_USB_CDC_NUM * (8 + 58) + (9 + 9 + 7 + 7))
...
#define MSC_MEDIA_PACKET                                                       \
  (2048) // was 8192; how low can it go whilst still working?
...
typedef struct {
  ...
  uint32_t
      ctl_packet_buf[CDC_DATA_MAX_PACKET_SIZE / 4]; // Force 32-bit alignment
  uint8_t iface_num;
  uint8_t in_ep;
  uint8_t out_ep;
  uint8_t cur_request;
...
typedef struct _usbd_cdc_msc_hid_state_t {
  ...
  __ALIGN_BEGIN uint8_t
      usbd_config_desc[MAX_TEMPLATE_CONFIG_DESC_SIZE] __ALIGN_END;
...
extern const uint8_t
    USBD_HID_KEYBOARD_ReportDesc[USBD_HID_KEYBOARD_REPORT_DESC_SIZE];
...
#if MICROPY_HW_USB_MSC
static inline void USBD_MSC_RegisterStorage(usbd_cdc_msc_hid_state_t *usbd,
...
uint8_t USBD_HID_SendReport(usbd_hid_state_t *usbd, uint8_t *report,
                            uint16_t len);
...
int8_t usbd_cdc_control(usbd_cdc_state_t *cdc, uint8_t cmd, uint8_t *pbuf,
                        uint16_t length);
