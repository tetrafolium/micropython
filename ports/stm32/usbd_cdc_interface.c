...
// Used to control the connect_state variable when USB host opens the serial
// port
...
int8_t usbd_cdc_control(usbd_cdc_state_t *cdc_in, uint8_t cmd, uint8_t *pbuf,
                        uint16_t length) {
  usbd_cdc_itf_t *cdc = (usbd_cdc_itf_t *)cdc_in;
...
static uint16_t usbd_cdc_tx_send_length(usbd_cdc_itf_t *cdc) {
  uint16_t to_end =
      USBD_CDC_TX_DATA_SIZE - usbd_cdc_tx_buffer_mask(cdc->tx_buf_ptr_out);
...
void usbd_cdc_tx_ready(usbd_cdc_state_t *cdc_in) {
  ...
  // According to the USB specification, a packet size of 64 bytes
  // (CDC_DATA_FS_MAX_PACKET_SIZE) gets held at the USB host until the next
  // packet is sent.  This is because a packet of maximum size is considered to
  // be part of a longer chunk of data, and the host waits for all data to
  // arrive (ie, waits for a packet < max packet size). To flush a packet of
  // exactly max packet size, we need to send a zero-size packet. See eg
  // http://www.cypress.com/?id=4&rID=92719
  cdc->tx_need_empty_packet =
      (len > 0 && len % usbd_cdc_max_packet(cdc->base.usbd->pdev) == 0 &&
       usbd_cdc_tx_buffer_will_be_empty(cdc));
}
...
void HAL_PCD_SOFCallback(PCD_HandleTypeDef *hpcd) {
  if (usbd_cdc_connect_tx_timer > 0) {
    ...
    usbd_cdc_msc_hid_state_t *usbd =
        ((USBD_HandleTypeDef *)hpcd->pData)->pClassData;
#if !MICROPY_HW_USB_IS_MULTI_OTG
    USB->CNTR &= ~USB_CNTR_SOFM;
#else
    hpcd->Instance->GINTMSK &= ~USB_OTG_GINTMSK_SOFM;
...
// len: number of bytes received into the buffer we passed to
// USBD_CDC_ReceivePacket Returns USBD_OK if all operations are OK else
// USBD_FAIL
int8_t usbd_cdc_receive(usbd_cdc_state_t *cdc_in, size_t len) {
  usbd_cdc_itf_t *cdc = (usbd_cdc_itf_t *)cdc_in;

  // copy the incoming data into the circular buffer
  for (const uint8_t *src = cdc->rx_packet_buf, *top = cdc->rx_packet_buf + len;
       src < top; ++src) {
  ...
  if ((cdc->flow & USBD_CDC_FLOWCONTROL_RTS) &&
      (usbd_cdc_rx_buffer_full(cdc))) {
...
int usbd_cdc_tx(usbd_cdc_itf_t *cdc, const uint8_t *buf, uint32_t len,
                uint32_t timeout) {
  for (uint32_t i = 0; i < len; i++) {
    // Wait until the device is connected and the buffer has space, with a given
    // timeout
    uint32_t start = HAL_GetTick();
    while (cdc->connect_state == USBD_CDC_CONNECT_STATE_DISCONNECTED ||
           usbd_cdc_tx_buffer_full(cdc)) {
      usbd_cdc_try_tx(cdc);
...
void usbd_cdc_tx_always(usbd_cdc_itf_t *cdc, const uint8_t *buf, uint32_t len) {
  for (int i = 0; i < len; i++) {
    // If the CDC device is not connected to the host then we don't have anyone
    // to receive our data. The device may become connected in the future, so we
    // should at least try to fill the buffer and hope that it doesn't overflow
    // by the time the device connects. If the device is not connected then we
    // should go ahead and fill the buffer straight away, ignoring overflow.
    // Otherwise, we should make sure that we have enough room in the buffer.
    if (cdc->connect_state != USBD_CDC_CONNECT_STATE_DISCONNECTED) {
      // If the buffer is full, wait until it gets drained, with a timeout of
      // 500ms (wraparound of tick is taken care of by 2's complement
      // arithmetic).
      uint32_t start = HAL_GetTick();
...
int usbd_cdc_rx(usbd_cdc_itf_t *cdc, uint8_t *buf, uint32_t len,
                uint32_t timeout) {
  // loop to read bytes
  for (uint32_t i = 0; i < len; i++) {
