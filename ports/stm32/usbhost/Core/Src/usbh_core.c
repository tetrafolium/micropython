...
USBH_Init(USBH_HandleTypeDef *phost,
          void (*pUsrFunc)(USBH_HandleTypeDef *phost, uint8_t), uint8_t id) {
  ...
  osThreadDef(USBH_Thread, USBH_Process_OS, USBH_PROCESS_PRIO, 0,
...
uint8_t USBH_FindInterface(USBH_HandleTypeDef *phost, uint8_t Class,
  ...
  if ((pif->bInterfaceClass == 0xFF) && (pif->bInterfaceSubClass == 0xFF) &&
      (pif->bInterfaceProtocol == 0xFF)) {
    return 0xFF;
  }
 ...
 *         Find the interface index for a specific class interface and alternate
 * setting number.
 * @param  phost: Host Handle
...
static USBH_StatusTypeDef USBH_HandleEnum(USBH_HandleTypeDef *phost) {
  ...
  case ENUM_GET_CFG_DESC:
    ...
    if (USBH_Get_CfgDesc(phost, USB_CONFIGURATION_DESC_SIZE) == USBH_OK) {
  ...
  case ENUM_GET_MFC_STRING_DESC:
    if (phost->device.DevDesc.iManufacturer !=
        0) { /* Check that Manufacturer String is available */

      if (USBH_Get_StringDesc(phost, phost->device.DevDesc.iManufacturer,
                              phost->device.Data, 0xff) == USBH_OK) {
  ...
  case ENUM_GET_PRODUCT_STRING_DESC:
    if (phost->device.DevDesc.iProduct !=
        0) { /* Check that Product string is available */
  ...
  case ENUM_GET_SERIALNUM_STRING_DESC:
    if (phost->device.DevDesc.iSerialNumber !=
        0) { /* Check that Serial number string is available */
...
static void USBH_Process_OS(void const *argument) {
  ...
  for (;;) {
    event =
        osMessageGet(((USBH_HandleTypeDef *)argument)->os_event, osWaitForever);
