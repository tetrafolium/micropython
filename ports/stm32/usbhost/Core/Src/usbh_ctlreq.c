...
static void USBH_ParseDevDesc(USBH_DevDescTypeDef *, uint8_t *buf,
                              uint16_t length);
...
static void USBH_ParseStringDesc(uint8_t *psrc, uint8_t *pdest,
                                 uint16_t length);
...
USBH_StatusTypeDef USBH_Get_DevDesc(USBH_HandleTypeDef *phost, uint8_t length) {
  ...
  if ((status = USBH_GetDescriptor(
           ...
           USB_DESC_DEVICE, phost->device.Data, length)) == USBH_OK) {
...
USBH_StatusTypeDef USBH_Get_StringDesc(USBH_HandleTypeDef *phost,
  ...
  if ((status = USBH_GetDescriptor(
           phost, USB_REQ_RECIPIENT_DEVICE | USB_REQ_TYPE_STANDARD,
           USB_DESC_STRING | string_index, phost->device.Data, length)) ==
      USBH_OK) {
    /* Commands successfully sent and Response Received  */
...
static void USBH_ParseDevDesc(USBH_DevDescTypeDef *dev_desc, uint8_t *buf,
  ...
  if (length > 8) { /* For 1st time after device connection, Host may issue only
                    8 bytes for Device Descriptor Length  */
...
static void USBH_ParseStringDesc(uint8_t *psrc, uint8_t *pdest,
  ...
  */
  ...
  if (psrc[1] ==
      USB_DESC_TYPE_STRING) { /* Make sure the Descriptor is String Type */

    /* psrc[0] contains Size of Descriptor, subtract 2 to get the length of
     * string */
    strlength = (((psrc[0] - 2) <= length) ? (psrc[0] - 2) : length);
    psrc +=
        2; /* Adjust the offset ignoring the String Len and Descriptor type */

    for (idx = 0; idx < strlength;
         idx += 2) { /* Copy Only the string and ignore the UNICODE ID, hence
                        add the src */
      *pdest = psrc[idx];
      pdest++;
    }
...
static USBH_StatusTypeDef USBH_HandleControl(USBH_HandleTypeDef *phost) {
  ...
  case CTRL_DATA_OUT_WAIT:
    ...
    if (URB_Status == USBH_URB_DONE) { /* If the Setup Pkt is sent successful,
  ...
  case CTRL_STATUS_IN_WAIT:
    ...
    if (URB_Status == USBH_URB_DONE) { /* Control transfers completed, Exit the
                                          State Machine */
