 ...
 *Definition for Human Interface Devices (CDC) Version 1.11 Jun 27, 2001". This
...
static USBH_StatusTypeDef USBH_CDC_InterfaceInit(USBH_HandleTypeDef *phost) {
  ...
  if (interface == 0xFF) /* No Valid Interface */
  {
    ...
    phost->pActiveClass->pData =
        (CDC_HandleTypeDef *)USBH_malloc(sizeof(CDC_HandleTypeDef));
    ...
    CDC_Handle->CommItf.NotifPipe =
        USBH_AllocPipe(phost, CDC_Handle->CommItf.NotifEp);
    ...
    interface = USBH_FindInterface(phost, DATA_INTERFACE_CLASS_CODE, RESERVED,
    ...
    if (interface == 0xFF) /* No Valid Interface */
    {
      ...
      if (phost->device.CfgDesc.Itf_Desc[interface]
              ...
              .bEndpointAddress &
          0x80) {
        CDC_Handle->DataItf.InEp = phost->device.CfgDesc.Itf_Desc[interface]
                                       .Ep_Desc[0]
                                       .bEndpointAddress;
        CDC_Handle->DataItf.InEpSize =
            phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[0].wMaxPacketSize;
      } else {
        CDC_Handle->DataItf.OutEp = phost->device.CfgDesc.Itf_Desc[interface]
                                        .Ep_Desc[0]
                                        .bEndpointAddress;
      ...
      CDC_Handle->DataItf.OutPipe =
          USBH_AllocPipe(phost, CDC_Handle->DataItf.OutEp);

      /*Allocate the length for host channel number in*/
      CDC_Handle->DataItf.InPipe =
          USBH_AllocPipe(phost, CDC_Handle->DataItf.InEp);

      /* Open channel for OUT endpoint */
      USBH_OpenPipe(phost, CDC_Handle->DataItf.OutPipe,
                    CDC_Handle->DataItf.OutEp, phost->device.address,
                    phost->device.speed, USB_EP_TYPE_BULK,
                    CDC_Handle->DataItf.OutEpSize);
...
static USBH_StatusTypeDef GetLineCoding(USBH_HandleTypeDef *phost,
  ...
  phost->Control.setup.b.bmRequestType =
      USB_D2H | USB_REQ_TYPE_CLASS | USB_REQ_RECIPIENT_INTERFACE;
...
static USBH_StatusTypeDef SetLineCoding(USBH_HandleTypeDef *phost,
  ...
  phost->Control.setup.b.bmRequestType =
      USB_H2D | USB_REQ_TYPE_CLASS | USB_REQ_RECIPIENT_INTERFACE;
...
/**
 * @brief  This function prepares the state before issuing the class specific
 * commands
 * @param  None
 * @retval None
 */
  ...
  case CDC_SEND_DATA:
    if (CDC_Handle->TxDataLength > CDC_Handle->DataItf.OutEpSize) {
      USBH_BulkSendData(phost, CDC_Handle->pTxData,
                        CDC_Handle->DataItf.OutEpSize,
                        CDC_Handle->DataItf.OutPipe, 1);
    } else {
      USBH_BulkSendData(phost, CDC_Handle->pTxData, CDC_Handle->TxDataLength,
                        CDC_Handle->DataItf.OutPipe, 1);
    }
