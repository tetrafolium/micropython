 ...
 *Version 1.0 Sep. 31, 1999". This driver implements the following aspects of
 *the specification:
...
static USBH_StatusTypeDef USBH_MSC_ClassRequest(USBH_HandleTypeDef *phost) {
  ...
  case MSC_REQ_GET_MAX_LUN:
    ...
    if (USBH_MSC_BOT_REQ_GetMaxLUN(phost, (uint8_t *)&MSC_Handle->max_lun) ==
        USBH_OK) {
      MSC_Handle->max_lun = (uint8_t)(MSC_Handle->max_lun) + 1;
      USBH_UsrLog("Number of supported LUN: %lu",
  ...
  case MSC_INIT:
    ...
    if (MSC_Handle->current_lun < MSC_Handle->max_lun) {
      ...
      case MSC_READ_INQUIRY:
        scsi_status = USBH_MSC_SCSI_Inquiry(
            phost, MSC_Handle->current_lun,
      ...
      case MSC_TEST_UNIT_READY:
        ready_status =
            USBH_MSC_SCSI_TestUnitReady(phost, MSC_Handle->current_lun);

        if (ready_status == USBH_OK) {
          if (MSC_Handle->unit[MSC_Handle->current_lun].prev_ready_state !=
              USBH_OK) {
      ...
      case MSC_READ_CAPACITY10:
        scsi_status = USBH_MSC_SCSI_ReadCapacity(
            phost, MSC_Handle->current_lun,
        ...
        if (scsi_status == USBH_OK) {
          if (MSC_Handle->unit[MSC_Handle->current_lun].state_changed == 1) {
            USBH_UsrLog("Mass Storage Device capacity : %lu MB",
                        (int32_t)((MSC_Handle->unit[MSC_Handle->current_lun]
                                       .capacity.block_nbr *
                                   MSC_Handle->unit[MSC_Handle->current_lun]
                                       .capacity.block_size) /
                                  1024 / 1024));
      ...
      case MSC_REQUEST_SENSE:
        scsi_status = USBH_MSC_SCSI_RequestSense(
            phost, MSC_Handle->current_lun,
        ...
        if (scsi_status == USBH_OK) {
          if ((MSC_Handle->unit[MSC_Handle->current_lun].sense.key ==
            ...
            if (phost->Timer <= MSC_Handle->timer) {
              MSC_Handle->unit[MSC_Handle->current_lun].state =
                  MSC_TEST_UNIT_READY;
              break;
            }
          }
...
static USBH_StatusTypeDef USBH_MSC_RdWrProcess(USBH_HandleTypeDef *phost,
  ...
  case MSC_REQUEST_SENSE:
    scsi_status =
        USBH_MSC_SCSI_RequestSense(phost, lun, &MSC_Handle->unit[lun].sense);
...
USBH_StatusTypeDef USBH_MSC_Read(USBH_HandleTypeDef *phost, uint8_t lun,
  ...
  if ((phost->device.is_connected == 0) || (phost->gState != HOST_CLASS) ||
      (MSC_Handle->unit[lun].state != MSC_IDLE)) {
    return USBH_FAIL;
  }
