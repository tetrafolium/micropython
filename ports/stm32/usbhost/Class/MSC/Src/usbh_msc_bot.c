...
USBH_StatusTypeDef USBH_MSC_BOT_Process(USBH_HandleTypeDef *phost,
  ...
  case BOT_SEND_CBW:
    ...
    USBH_BulkSendData(phost, MSC_Handle->hbot.cbw.data, BOT_CBW_LENGTH,
                      MSC_Handle->OutPipe, 1);
  ...
  case BOT_SEND_CBW_WAIT:
    ...
    if (URB_Status == USBH_URB_DONE) {
      if (MSC_Handle->hbot.cbw.field.DataTransferLength != 0) {
        ...
        if (((MSC_Handle->hbot.cbw.field.Flags) & USB_REQ_DIR_MASK) ==
            USB_D2H) {
  ...
  case BOT_DATA_IN:
    /* Send first packet */
    USBH_BulkReceiveData(phost, MSC_Handle->hbot.pbuf, MSC_Handle->InEpSize,
                         MSC_Handle->InPipe);
  ...
  case BOT_DATA_IN_WAIT:
    ...
    if (URB_Status == USBH_URB_DONE) {
      ...
      if (MSC_Handle->hbot.cbw.field.DataTransferLength >
          MSC_Handle->InEpSize) {
  ...
  case BOT_RECEIVE_CSW:
    ...
    USBH_BulkReceiveData(phost, MSC_Handle->hbot.csw.data, BOT_CSW_LENGTH,
                         MSC_Handle->InPipe);
...
static BOT_CSWStatusTypeDef USBH_MSC_DecodeCSW(USBH_HandleTypeDef *phost) {
  ...
  if (USBH_LL_GetLastXferSize(phost, MSC_Handle->InPipe) != BOT_CSW_LENGTH) {
    ...
    if (MSC_Handle->hbot.csw.field.Signature ==
        BOT_CSW_SIGNATURE) { /* Check Condition 1. dCSWSignature is equal to
                                53425355h */
