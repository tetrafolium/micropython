...
USBH_AUDIO_FindAudioStreamingIN(USBH_HandleTypeDef *phost);
static USBH_StatusTypeDef
USBH_AUDIO_FindAudioStreamingOUT(USBH_HandleTypeDef *phost);
static USBH_StatusTypeDef USBH_AUDIO_FindHIDControl(USBH_HandleTypeDef *phost);
static USBH_StatusTypeDef
USBH_AUDIO_ParseCSDescriptors(USBH_HandleTypeDef *phost);

static USBH_StatusTypeDef
USBH_AUDIO_BuildHeadphonePath(USBH_HandleTypeDef *phost);
static USBH_StatusTypeDef
USBH_AUDIO_BuildMicrophonePath(USBH_HandleTypeDef *phost);
...
static USBH_StatusTypeDef
ParseCSDescriptors(AUDIO_ClassSpecificDescTypedef *class_desc,
                   uint8_t ac_subclass, uint8_t *pdesc);
...
static USBH_StatusTypeDef USBH_AC_SetCur(USBH_HandleTypeDef *phost,
                                         uint8_t subtype, uint8_t feature,
...
static USBH_StatusTypeDef
USBH_AUDIO_SetEndpointControls(USBH_HandleTypeDef *phost, uint8_t Ep,
                               uint8_t *buff);

static USBH_StatusTypeDef AUDIO_SetVolume(USBH_HandleTypeDef *phost,
...
static USBH_StatusTypeDef
USBH_AUDIO_SetControlAttribute(USBH_HandleTypeDef *phost, uint8_t attrib);
static int32_t USBH_AUDIO_FindLinkedUnit(USBH_HandleTypeDef *phost,
                                         uint8_t UnitID);
...
static USBH_StatusTypeDef USBH_AUDIO_InterfaceInit(USBH_HandleTypeDef *phost) {
  ...
  interface =
      USBH_FindInterface(phost, AC_CLASS, USB_SUBCLASS_AUDIOCONTROL, 0x00);
  ...
  if (interface == 0xFF) /* Not Valid Interface */
  {
    ...
    phost->pActiveClass->pData =
        (AUDIO_HandleTypeDef *)USBH_malloc(sizeof(AUDIO_HandleTypeDef));
    ...
    if ((out_status == USBH_FAIL) && (in_status == USBH_FAIL)) {
      USBH_DbgLog("%s class configuration not supported.",
                  phost->pActiveClass->Name);
    } else {
      /* 2nd Step:  Select Audio Streaming Interfaces with largest endpoint size
       * : default behavior*/
        ...
        if (AUDIO_Handle->stream_in[index].valid == 1) {
          if (ep_size_in < AUDIO_Handle->stream_in[index].EpSize) {
            ...
            AUDIO_Handle->microphone.interface =
                AUDIO_Handle->stream_in[index].interface;
            AUDIO_Handle->microphone.AltSettings =
                AUDIO_Handle->stream_in[index].AltSettings;
            AUDIO_Handle->microphone.Ep = AUDIO_Handle->stream_in[index].Ep;
            AUDIO_Handle->microphone.EpSize =
                AUDIO_Handle->stream_in[index].EpSize;
            AUDIO_Handle->microphone.Poll =
                AUDIO_Handle->stream_out[index].Poll;
      ...
      if (AUDIO_Handle->headphone.supported == 1) {
        ...
        AUDIO_Handle->headphone.Pipe =
            USBH_AllocPipe(phost, AUDIO_Handle->headphone.Ep);

        /* Open pipe for IN endpoint */
        USBH_OpenPipe(phost, AUDIO_Handle->headphone.Pipe,
                      AUDIO_Handle->headphone.Ep, phost->device.address,
                      phost->device.speed, USB_EP_TYPE_ISOC,
                      AUDIO_Handle->headphone.EpSize);
      ...
      if (AUDIO_Handle->microphone.supported == 1) {
        ...
        AUDIO_Handle->microphone.Pipe =
            USBH_AllocPipe(phost, AUDIO_Handle->microphone.Ep);

        /* Open pipe for IN endpoint */
        USBH_OpenPipe(phost, AUDIO_Handle->microphone.Pipe,
                      AUDIO_Handle->microphone.Ep, phost->device.address,
                      phost->device.speed, USB_EP_TYPE_ISOC,
                      AUDIO_Handle->microphone.EpSize);
      ...
      if (AUDIO_Handle->control.supported == 1) {
        AUDIO_Handle->control.Pipe =
            USBH_AllocPipe(phost, AUDIO_Handle->control.Ep);

        /* Open pipe for IN endpoint */
        USBH_OpenPipe(phost, AUDIO_Handle->control.Pipe,
                      AUDIO_Handle->control.Ep, phost->device.address,
...
static USBH_StatusTypeDef USBH_AUDIO_ClassRequest(USBH_HandleTypeDef *phost) {
  ...
  case AUDIO_REQ_SET_DEFAULT_IN_INTERFACE:
    if (AUDIO_Handle->microphone.supported == 1) {
      req_status =
          USBH_SetInterface(phost, AUDIO_Handle->microphone.interface, 0);
  ...
  case AUDIO_REQ_SET_DEFAULT_OUT_INTERFACE:
    if (AUDIO_Handle->headphone.supported == 1) {
      req_status =
          USBH_SetInterface(phost, AUDIO_Handle->headphone.interface, 0);
      ...
      if (req_status == USBH_OK) {
        ...
        AUDIO_Handle->temp_channels =
            AUDIO_Handle->headphone.asociated_channels;
      }
    } else {
      AUDIO_Handle->req_state = AUDIO_REQ_CS_REQUESTS;
      AUDIO_Handle->cs_req_state = AUDIO_REQ_GET_VOLUME;
  ...
  case AUDIO_REQ_SET_IN_INTERFACE:
    if (AUDIO_Handle->microphone.supported == 1) {
      req_status = USBH_SetInterface(phost, AUDIO_Handle->microphone.interface,
                                     AUDIO_Handle->microphone.AltSettings);
  ...
  case AUDIO_REQ_SET_OUT_INTERFACE:
    if (AUDIO_Handle->headphone.supported == 1) {
      req_status = USBH_SetInterface(phost, AUDIO_Handle->headphone.interface,
                                     AUDIO_Handle->headphone.AltSettings);
 ...
 *         The function is responsible for handling AC Specific requests for a
 * specific feature and channel for Audio class.
 * @param  phost: Host handle
 * @retval USBH Status
 */
static USBH_StatusTypeDef USBH_AUDIO_CSRequest(USBH_HandleTypeDef *phost,
  ...
  case AUDIO_REQ_GET_VOLUME:
    req_status = USBH_AC_GetCur(phost, UAC_FEATURE_UNIT, /* subtype  */
                                feature,                 /* feature  */
  ...
  case AUDIO_REQ_GET_MIN_VOLUME:
    req_status = USBH_AC_GetMin(phost, UAC_FEATURE_UNIT, /* subtype  */
                                feature,                 /* feature  */
                                VOLUME_CONTROL,          /* Selector */
                                channel,                 /* channel  */
                                0x02);                   /* length   */
  ...
  case AUDIO_REQ_GET_MAX_VOLUME:
    req_status = USBH_AC_GetMax(phost, UAC_FEATURE_UNIT, /* subtype  */
                                feature,                 /* feature  */
                                VOLUME_CONTROL,          /* Selector */
                                channel,                 /* channel  */
                                0x02);                   /* length   */
    if (req_status != USBH_BUSY) {
      ...
      if (AUDIO_Handle->headphone.attribute.volumeMax <
  ...
  case AUDIO_REQ_GET_RESOLUTION:
    req_status = USBH_AC_GetRes(phost, UAC_FEATURE_UNIT, /* subtype  */
                                feature,                 /* feature  */
                                VOLUME_CONTROL,          /* Selector */
                                channel,                 /* channel  */
                                0x02);                   /* length   */
 ...
 *         The function is responsible for handling AC Specific requests for a
 * all features and associated channels for Audio class.
 * @param  phost: Host handle
 * @retval USBH Status
 */
static USBH_StatusTypeDef
USBH_AUDIO_HandleCSRequest(USBH_HandleTypeDef *phost) {

  USBH_StatusTypeDef status = USBH_BUSY;
  USBH_StatusTypeDef cs_status = USBH_BUSY;
  AUDIO_HandleTypeDef *AUDIO_Handle = phost->pActiveClass->pData;

  cs_status = USBH_AUDIO_CSRequest(phost, AUDIO_Handle->temp_feature,
                                   AUDIO_Handle->temp_channels);
    ...
    if ((phost->device.CfgDesc.Itf_Desc[interface].bInterfaceClass ==
      ...
      if ((phost->device.CfgDesc.Itf_Desc[interface]
          ...
          (phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[0].wMaxPacketSize >
           0)) {
        AUDIO_Handle->stream_in[alt_settings].Ep =
            phost->device.CfgDesc.Itf_Desc[interface]
                .Ep_Desc[0]
                .bEndpointAddress;
        AUDIO_Handle->stream_in[alt_settings].EpSize =
            phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[0].wMaxPacketSize;
        AUDIO_Handle->stream_in[alt_settings].interface =
            phost->device.CfgDesc.Itf_Desc[interface].bInterfaceNumber;
        AUDIO_Handle->stream_in[alt_settings].AltSettings =
            phost->device.CfgDesc.Itf_Desc[interface].bAlternateSetting;
        AUDIO_Handle->stream_in[alt_settings].Poll =
  ...
  for (interface = 0; interface < USBH_MAX_NUM_INTERFACES; interface++) {
    if ((phost->device.CfgDesc.Itf_Desc[interface].bInterfaceClass ==
      ...
      if (((phost->device.CfgDesc.Itf_Desc[interface]
          ...
          (phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[0].wMaxPacketSize >
           0)) {
        AUDIO_Handle->stream_out[alt_settings].Ep =
            phost->device.CfgDesc.Itf_Desc[interface]
                .Ep_Desc[0]
                .bEndpointAddress;
        AUDIO_Handle->stream_out[alt_settings].EpSize =
            phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[0].wMaxPacketSize;
        AUDIO_Handle->stream_out[alt_settings].interface =
            phost->device.CfgDesc.Itf_Desc[interface].bInterfaceNumber;
        AUDIO_Handle->stream_out[alt_settings].AltSettings =
            phost->device.CfgDesc.Itf_Desc[interface].bAlternateSetting;
        AUDIO_Handle->stream_out[alt_settings].Poll =
            phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[0].bInterval;
...
static USBH_StatusTypeDef USBH_AUDIO_FindHIDControl(USBH_HandleTypeDef *phost) {
  ...
  interface =
      USBH_FindInterface(phost, AC_CLASS, USB_SUBCLASS_AUDIOCONTROL, 0xFF);
  if (interface != 0xFF) {
    for (interface = 0; interface < USBH_MAX_NUM_INTERFACES; interface++) {
      if ((phost->device.CfgDesc.Itf_Desc[interface].bInterfaceClass ==
           0x03) && /*HID*/
        ...
        if ((phost->device.CfgDesc.Itf_Desc[interface]
                 ...
                 .bEndpointAddress &
             0x80) == 0x80) {
          AUDIO_Handle->control.Ep = phost->device.CfgDesc.Itf_Desc[interface]
                                         .Ep_Desc[0]
                                         .bEndpointAddress;
          AUDIO_Handle->control.EpSize =
              phost->device.CfgDesc.Itf_Desc[interface]
                  .Ep_Desc[0]
                  .wMaxPacketSize;
          AUDIO_Handle->control.interface =
              phost->device.CfgDesc.Itf_Desc[interface].bInterfaceNumber;
          AUDIO_Handle->control.Poll =
              phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[0].bInterval;
          AUDIO_Handle->control.supported = 1;
          status = USBH_OK;
          break;
...
ParseCSDescriptors(AUDIO_ClassSpecificDescTypedef *class_desc,
  ...
  if (ac_subclass == USB_SUBCLASS_AUDIOCONTROL) {
    ...
    case UAC_INPUT_TERMINAL:
      class_desc->cs_desc.InputTerminalDesc[class_desc->InputTerminalNum++] =
          (AUDIO_ITDescTypeDef *)pdesc;
      break;

    case UAC_OUTPUT_TERMINAL:
      class_desc->cs_desc.OutputTerminalDesc[class_desc->OutputTerminalNum++] =
          (AUDIO_OTDescTypeDef *)pdesc;
      break;

    case UAC_FEATURE_UNIT:
      class_desc->cs_desc.FeatureUnitDesc[class_desc->FeatureUnitNum++] =
          (AUDIO_FeatureDescTypeDef *)pdesc;
      break;

    case UAC_SELECTOR_UNIT:
      class_desc->cs_desc.SelectorUnitDesc[class_desc->SelectorUnitNum++] =
          (AUDIO_SelectorDescTypeDef *)pdesc;
      break;

    case UAC_MIXER_UNIT:
      class_desc->cs_desc.MixerUnitDesc[class_desc->MixerUnitNum++] =
          (AUDIO_MixerDescTypeDef *)pdesc;
      break;
    ...
    switch (pdesc[2]) {
    case UAC_AS_GENERAL:
      class_desc->as_desc[class_desc->ASNum].GeneralDesc =
          (AUDIO_ASGeneralDescTypeDef *)pdesc;
      break;
    case UAC_FORMAT_TYPE:
      class_desc->as_desc[class_desc->ASNum++].FormatTypeDesc =
          (AUDIO_ASFormatTypeDescTypeDef *)pdesc;
      break;
    default:
      break;
    }
...
static int32_t USBH_AUDIO_FindLinkedUnit(USBH_HandleTypeDef *phost,
  ...
  for (Index = 0; Index < AUDIO_Handle->class_desc.FeatureUnitNum; Index++) {
    if (AUDIO_Handle->class_desc.cs_desc.FeatureUnitDesc[Index]->bSourceID ==
        UnitID) {
  ...
  for (Index = 0; Index < AUDIO_Handle->class_desc.MixerUnitNum; Index++) {
    if ((AUDIO_Handle->class_desc.cs_desc.MixerUnitDesc[Index]->bSourceID0 ==
         UnitID) ||
        (AUDIO_Handle->class_desc.cs_desc.MixerUnitDesc[Index]->bSourceID1 ==
         UnitID)) {
  ...
  for (Index = 0; Index < AUDIO_Handle->class_desc.SelectorUnitNum; Index++) {
    if (AUDIO_Handle->class_desc.cs_desc.SelectorUnitDesc[Index]->bSourceID0 ==
        UnitID) {
      UnitID =
          AUDIO_Handle->class_desc.cs_desc.SelectorUnitDesc[Index]->bUnitID;
  ...
  for (Index = 0; Index < AUDIO_Handle->class_desc.OutputTerminalNum; Index++) {
    if (AUDIO_Handle->class_desc.cs_desc.OutputTerminalDesc[Index]->bSourceID ==
        UnitID) {
      UnitID = AUDIO_Handle->class_desc.cs_desc.OutputTerminalDesc[Index]
                   ->bTerminalID;
...
USBH_StatusTypeDef USBH_AUDIO_BuildMicrophonePath(USBH_HandleTypeDef *phost) {
  ...
  for (terminalIndex = 0;
       ...
       terminalIndex++) {
    if (LE16(AUDIO_Handle->class_desc.cs_desc.InputTerminalDesc[terminalIndex]
                 ->wTerminalType) == 0x201) {
      UnitID =
          AUDIO_Handle->class_desc.cs_desc.InputTerminalDesc[terminalIndex]
              ->bTerminalID;
      AUDIO_Handle->microphone.asociated_channels =
          AUDIO_Handle->class_desc.cs_desc.InputTerminalDesc[terminalIndex]
              ->bNrChannels;
      break;
    }
...
USBH_StatusTypeDef USBH_AUDIO_BuildHeadphonePath(USBH_HandleTypeDef *phost) {
  ...
  for (Index = 0; Index < AUDIO_Handle->class_desc.ASNum; Index++) {
    if (AUDIO_Handle->class_desc.as_desc[Index].GeneralDesc->bTerminalLink ==
        UnitID) {
  ...
  do {
    ...
    case UAC_OUTPUT_TERMINAL:
      ...
      if (LE16(AUDIO_Handle->class_desc.cs_desc.OutputTerminalDesc[Index]
                   ->wTerminalType) != 0x103) {
        return USBH_OK;
      }
      break;
...
static USBH_StatusTypeDef USBH_AC_GetCur(USBH_HandleTypeDef *phost,
  ...
  case UAC_OUTPUT_TERMINAL:
    UnitID =
        AUDIO_Handle->class_desc.cs_desc.OutputTerminalDesc[0]->bTerminalID;
    InterfaceNum = 0; /*Always zero Control Interface */
    wIndex = (UnitID << 8) | InterfaceNum;
    wValue = (COPY_PROTECT_CONTROL << 8);
    wLength = 1;
...
USBH_AUDIO_SetEndpointControls(USBH_HandleTypeDef *phost, uint8_t Ep,
  ...
  phost->Control.setup.b.bmRequestType =
      USB_H2D | USB_REQ_RECIPIENT_ENDPOINT | USB_REQ_TYPE_CLASS;
...
static USBH_StatusTypeDef USBH_AUDIO_Control(USBH_HandleTypeDef *phost) {
  ...
  case AUDIO_CONTROL_INIT:
    if ((phost->Timer & 1) == 0) {
      ...
      USBH_InterruptReceiveData(phost, (uint8_t *)(AUDIO_Handle->mem),
                                AUDIO_Handle->control.EpSize,
                                AUDIO_Handle->control.Pipe);
  ...
  case AUDIO_CONTROL_CHANGE:
    if (USBH_LL_GetURBState(phost, AUDIO_Handle->control.Pipe) ==
        USBH_URB_DONE) {
    ...
    if ((phost->Timer - AUDIO_Handle->control.timer) >=
        AUDIO_Handle->control.Poll) {
      AUDIO_Handle->control.timer = phost->Timer;
...
static USBH_StatusTypeDef USBH_AUDIO_OutputStream(USBH_HandleTypeDef *phost) {
  ...
  case AUDIO_PLAYBACK_INIT:
    ...
    if (AUDIO_Handle->class_desc.as_desc[AUDIO_Handle->headphone.asociated_as]
            .FormatTypeDesc->bSamFreqType == 0) {
  ...
  case AUDIO_PLAYBACK_SET_EP_FREQ:
    ...
    buff = (uint8_t *)AUDIO_Handle->class_desc
               .as_desc[AUDIO_Handle->headphone.asociated_as]
               .FormatTypeDesc->tSamFreq[0];

    status =
        USBH_AUDIO_SetEndpointControls(phost, AUDIO_Handle->headphone.Ep, buff);
    if (status == USBH_OK) {
...
static USBH_StatusTypeDef USBH_AUDIO_Transmit(USBH_HandleTypeDef *phost) {
  ...
  case AUDIO_DATA_START_OUT:
    ...
    if ((phost->Timer & 1) == 0) {
      ...
      USBH_IsocSendData(phost, AUDIO_Handle->headphone.buf,
                        AUDIO_Handle->headphone.frame_length,
                        AUDIO_Handle->headphone.Pipe);

      AUDIO_Handle->headphone.partial_ptr =
          AUDIO_Handle->headphone.frame_length;
  ...
  case AUDIO_DATA_OUT:
    if ((USBH_LL_GetURBState(phost, AUDIO_Handle->headphone.Pipe) ==
         USBH_URB_DONE) &&
        ((phost->Timer - AUDIO_Handle->headphone.timer) >=
      ...
      if (AUDIO_Handle->headphone.global_ptr <=
          AUDIO_Handle->headphone.total_length) {
        USBH_IsocSendData(phost, AUDIO_Handle->headphone.cbuf,
                          AUDIO_Handle->headphone.frame_length,
                          AUDIO_Handle->headphone.Pipe);
        ...
        AUDIO_Handle->headphone.partial_ptr +=
            AUDIO_Handle->headphone.frame_length;
        AUDIO_Handle->headphone.global_ptr +=
            AUDIO_Handle->headphone.frame_length;
      } else {
...
USBH_StatusTypeDef USBH_AUDIO_SetFrequency(USBH_HandleTypeDef *phost,
  ...
  if (phost->gState == HOST_CLASS) {
    ...
    if (AUDIO_Handle->play_state == AUDIO_PLAYBACK_IDLE) {
      ...
      if (AUDIO_Handle->class_desc.as_desc[AUDIO_Handle->headphone.asociated_as]
              .FormatTypeDesc->bSamFreqType == 0) {
        freq_min = LE24(AUDIO_Handle->class_desc
                            .as_desc[AUDIO_Handle->headphone.asociated_as]
                            .FormatTypeDesc->tSamFreq[0]);
        freq_max = LE24(AUDIO_Handle->class_desc
                            .as_desc[AUDIO_Handle->headphone.asociated_as]
                            .FormatTypeDesc->tSamFreq[1]);
        ...
        num_supported_freq = (AUDIO_Handle->class_desc
                                  .as_desc[AUDIO_Handle->headphone.asociated_as]
                                  .FormatTypeDesc->bLength -
                              8) /
                             3;

        for (index = 0; index < num_supported_freq; index++) {
          if (SampleRate ==
              LE24(AUDIO_Handle->class_desc
                       .as_desc[AUDIO_Handle->headphone.asociated_as]
                       .FormatTypeDesc->tSamFreq[index])) {
            change_freq = TRUE;
            break;
          }
        }
      ...
      if (change_freq == TRUE) {
        ...
        AUDIO_Handle->headphone.frame_length =
            (SampleRate * BitPerSample * NbrChannels) / 8000;
        AUDIO_Handle->play_state = AUDIO_PLAYBACK_SET_EP;
        Status = USBH_OK;
      }
    }
 ...
 * @retval USBH Status
 */
USBH_StatusTypeDef USBH_AUDIO_Play(USBH_HandleTypeDef *phost, uint8_t *buf,
                                   uint32_t length) {
  USBH_StatusTypeDef Status = USBH_FAIL;
  AUDIO_HandleTypeDef *AUDIO_Handle;
 ...
 * @retval USBH Status
 */
USBH_StatusTypeDef USBH_AUDIO_ChangeOutBuffer(USBH_HandleTypeDef *phost,
                                              uint8_t *buf) {
  USBH_StatusTypeDef Status = USBH_FAIL;
  AUDIO_HandleTypeDef *AUDIO_Handle;
...
USBH_AUDIO_SetControlAttribute(USBH_HandleTypeDef *phost, uint8_t attrib) {
  ...
  if (AUDIO_Handle->headphone.attribute.volume >
  ...
  if (AUDIO_SetVolume(phost, AUDIO_Handle->temp_feature,
                      AUDIO_Handle->temp_channels,
 ...
 * @retval USBH Status
 */
USBH_StatusTypeDef USBH_AUDIO_SetVolume(USBH_HandleTypeDef *phost,
                                        AUDIO_VolumeCtrlTypeDef volume_ctl) {
  AUDIO_HandleTypeDef *AUDIO_Handle = phost->pActiveClass->pData;
  ...
  if ((volume_ctl == VOLUME_UP) || (volume_ctl == VOLUME_DOWN)) {
    if (phost->gState == HOST_CLASS) {
      AUDIO_Handle = phost->pActiveClass->pData;
      if (AUDIO_Handle->play_state == AUDIO_PLAYBACK_PLAY) {
        AUDIO_Handle->control_state = (volume_ctl == VOLUME_UP)
...
static USBH_StatusTypeDef AUDIO_SetVolume(USBH_HandleTypeDef *phost,
  ...
  status = USBH_AC_SetCur(phost, UAC_FEATURE_UNIT, feature, VOLUME_CONTROL,
                          channel, 2);
