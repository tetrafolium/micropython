...
static USBH_StatusTypeDef USBH_HID_InterfaceInit(USBH_HandleTypeDef *phost) {
  ...
  if (interface == 0xFF) /* No Valid Interface */
  {
    ...
    } else if (phost->device.CfgDesc.Itf_Desc[phost->device.current_interface]
    ...
    max_ep =
        ((phost->device.CfgDesc.Itf_Desc[phost->device.current_interface]
              .bNumEndpoints <= USBH_MAX_NUM_ENDPOINTS)
             ? phost->device.CfgDesc.Itf_Desc[phost->device.current_interface]
                   .bNumEndpoints
...
static USBH_StatusTypeDef USBH_HID_ClassRequest(USBH_HandleTypeDef *phost) {
  ...
  case HID_REQ_GET_REPORT_DESC:
    ...
    /* Get Report Desc */
    if (USBH_HID_GetHIDReportDescriptor(
            phost, HID_Handle->HID_Desc.wItemLength) == USBH_OK) {
...
static USBH_StatusTypeDef USBH_HID_Process(USBH_HandleTypeDef *phost) {
  ...
  case HID_POLL:
    ...
    } else if (USBH_LL_GetURBState(phost, HID_Handle->InPipe) ==
...
USBH_StatusTypeDef USBH_HID_GetHIDReportDescriptor(USBH_HandleTypeDef *phost,
  ...
  status = USBH_GetDescriptor(
      phost, USB_REQ_RECIPIENT_INTERFACE | USB_REQ_TYPE_STANDARD,
...
USBH_StatusTypeDef USBH_HID_GetHIDDescriptor(USBH_HandleTypeDef *phost,
  ...
  status = USBH_GetDescriptor(
      phost, USB_REQ_RECIPIENT_INTERFACE | USB_REQ_TYPE_STANDARD, USB_DESC_HID,
