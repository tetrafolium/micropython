...
/* 0E    Reserved */
#define HID_USAGE_PAGE_PID                                                     \
  uint16_t(0x0F) /* PID Page (force feedback and related devices) */
...
#define HID_USAGE_PAGE_MSR                                                     \
  uint16_t(0x8E) /* Magnetic Stripe Reading (MSR) Devices */
...
#define HID_USAGE_JOYSTICK                                                     \
  uint16_t(0x04)                         /* Joystick (Application Collection) */
...
#define HID_USAGE_MAX_CTR                                                      \
  uint16_t(0x08) /* Multi-axis Controller (Application Collection) */
/* 09-2F Reserved */
...
#define HID_USAGE_COUNTEDBUF                                                   \
  uint16_t(0x3A) /* Counted Buffer (Logical Collection) */
#define HID_USAGE_BYTECOUNT uint16_t(0x3B) /* Byte Count (Dynamic Value) */
#define HID_USAGE_MOTIONWAKE                                                   \
  uint16_t(0x3C)                        /* Motion Wakeup (One Shot Control) */
...
#define HID_USAGE_FEATNOTIF                                                    \
  uint16_t(0x47) /* Feature Notification (Dynamic Value),(Dynamic Flag) */
/* 48-7F Reserved */
#define HID_USAGE_SYSCTL                                                       \
  uint16_t(0x80) /* System Control (Application Collection) */
#define HID_USAGE_PWDOWN                                                       \
  uint16_t(0x81) /* System Power Down (One Shot Control) */
...
#define HID_USAGE_WAKEUP uint16_t(0x83) /* System Wake Up (One Shot Control)   \
                                         */
#define HID_USAGE_CONTEXTM                                                     \
  uint16_t(0x84) /* System Context Menu (One Shot Control) */
...
#define HID_USAGE_MENUHELP                                                     \
  uint16_t(0x87) /* System Menu Help (One Shot Control) */
#define HID_USAGE_MENUEXIT                                                     \
  uint16_t(0x88) /* System Menu Exit (One Shot Control) */
#define HID_USAGE_MENUSELECT                                                   \
  uint16_t(0x89) /* System Menu Select (One Shot Control) */
#define HID_USAGE_SYSM_RIGHT                                                   \
  uint16_t(0x8A) /* System Menu Right (Re-Trigger Control) */
#define HID_USAGE_SYSM_LEFT                                                    \
  uint16_t(0x8B) /* System Menu Left (Re-Trigger Control) */
#define HID_USAGE_SYSM_UP                                                      \
  uint16_t(0x8C) /* System Menu Up (Re-Trigger Control) */
#define HID_USAGE_SYSM_DOWN                                                    \
  uint16_t(0x8D) /* System Menu Down (Re-Trigger Control) */
#define HID_USAGE_COLDRESET                                                    \
  uint16_t(0x8E) /* System Cold Restart (One Shot Control) */
#define HID_USAGE_WARMRESET                                                    \
  uint16_t(0x8F) /* System Warm Restart (One Shot Control) */
...
#define HID_USAGE_SYS_UNDOCK                                                   \
  uint16_t(0xA1) /* System Undock (One Shot Control) */
...
#define HID_USAGE_SYS_BREAK uint16_t(0xA3) /* System Break (One Shot Control)  \
                                            */
#define HID_USAGE_SYS_DBGBRK                                                   \
  uint16_t(0xA4) /* System Debugger Break (One Shot Control) */
#define HID_USAGE_APP_BRK                                                      \
  uint16_t(0xA5) /* Application Break (One Shot Control) */
#define HID_USAGE_APP_DBGBRK                                                   \
  uint16_t(0xA6) /* Application Debugger Break (One Shot Control) */
#define HID_USAGE_SYS_SPKMUTE                                                  \
  uint16_t(0xA7) /* System Speaker Mute (One Shot Control) */
#define HID_USAGE_SYS_HIBERN                                                   \
  uint16_t(0xA8) /* System Hibernate (One Shot Control) */
/* A9-AF Reserved */
#define HID_USAGE_SYS_SIDPINV                                                  \
  uint16_t(0xB0) /* System Display Invert (One Shot Control) */
#define HID_USAGE_SYS_DISPINT                                                  \
  uint16_t(0xB1) /* System Display Internal (One Shot Control) */
#define HID_USAGE_SYS_DISPEXT                                                  \
  uint16_t(0xB2) /* System Display External (One Shot Control) */
#define HID_USAGE_SYS_DISPBOTH                                                 \
  uint16_t(0xB3) /* System Display Both (One Shot Control) */
#define HID_USAGE_SYS_DISPDUAL                                                 \
  uint16_t(0xB4) /* System Display Dual (One Shot Control) */
#define HID_USAGE_SYS_DISPTGLIE                                                \
  uint16_t(0xB5) /* System Display Toggle Int/Ext (One Shot Control) */
#define HID_USAGE_SYS_DISP_SWAP                                                \
  uint16_t(0xB6) /* System Display Swap Primary/Secondary (One Shot Control)   \
                  */
#define HID_USAGE_SYS_DIPS_LCDA                                                \
  uint16_t(0xB7) /* System Display LCD Autoscale (One Shot Control) */
/* B8-FFFF Reserved */
