...
static void PTP_GetStorageInfo(USBH_HandleTypeDef *phost, uint32_t storage_id,
...
USBH_StatusTypeDef USBH_PTP_Process(USBH_HandleTypeDef *phost) {
  ...
  case PTP_OP_REQUEST_STATE:
    USBH_BulkSendData(phost, (uint8_t *)&(MTP_Handle->ptp.op_container),
                      MTP_Handle->ptp.op_container.length,
                      MTP_Handle->DataOutPipe, 1);
  ...
  case PTP_DATA_OUT_PHASE_STATE:
    ...
    USBH_BulkSendData(phost, MTP_Handle->ptp.data_ptr,
                      MTP_Handle->DataOutEpSize, MTP_Handle->DataOutPipe, 1);
  ...
  case PTP_DATA_OUT_PHASE_WAIT_STATE:
    ...
    if (URB_Status == USBH_URB_DONE) {
      ...
      if (MTP_Handle->ptp.data_length > 0) {
        USBH_BulkSendData(phost, MTP_Handle->ptp.data_ptr,
                          MTP_Handle->DataOutEpSize, MTP_Handle->DataOutPipe,
                          1);
      } else {
  ...
  case PTP_DATA_IN_PHASE_STATE:
    /* Send first packet */
    USBH_BulkReceiveData(phost, MTP_Handle->ptp.data_ptr,
                         MTP_Handle->DataInEpSize, MTP_Handle->DataInPipe);
  ...
  case PTP_DATA_IN_PHASE_WAIT_STATE:
    ...
    if (URB_Status == USBH_URB_DONE) {
      ...
      if ((len >= MTP_Handle->DataInEpSize) &&
          (MTP_Handle->ptp.data_length > 0)) {
  ...
  case PTP_RESPONSE_STATE:
    ...
    USBH_BulkReceiveData(phost, (uint8_t *)&(MTP_Handle->ptp.resp_container),
...
USBH_StatusTypeDef USBH_PTP_SendRequest(USBH_HandleTypeDef *phost,
  ...
  USBH_memset(&(MTP_Handle->ptp.op_container), 0,
              sizeof(PTP_OpContainerTypedef));
...
static void PTP_DecodeDeviceInfo(USBH_HandleTypeDef *phost,
  ...
  if (MTP_Handle->ptp.iteration == 0) {
    ...
    dev_info->VendorExtensionVersion =
        LE16(&data[PTP_di_VendorExtensionVersion]);
    PTP_GetString(dev_info->VendorExtensionDesc,
                  &data[PTP_di_VendorExtensionDesc], &len);

    totallen = len * 2 + 1;
    dev_info->FunctionalMode = LE16(&data[PTP_di_FunctionalMode + totallen]);
    dev_info->OperationsSupported_len =
        PTP_GetArray16((uint16_t *)&dev_info->OperationsSupported, data,
                       PTP_di_OperationsSupported + totallen);

    totallen = totallen + dev_info->OperationsSupported_len * sizeof(uint16_t) +
               sizeof(uint32_t);
    dev_info->EventsSupported_len =
        PTP_GetArray16((uint16_t *)&dev_info->EventsSupported, data,
                       PTP_di_OperationsSupported + totallen);

    totallen = totallen + dev_info->EventsSupported_len * sizeof(uint16_t) +
               sizeof(uint32_t);
    dev_info->DevicePropertiesSupported_len =
        PTP_GetArray16((uint16_t *)&dev_info->DevicePropertiesSupported, data,
                       PTP_di_OperationsSupported + totallen);

    totallen = totallen +
               dev_info->DevicePropertiesSupported_len * sizeof(uint16_t) +
               sizeof(uint32_t);

    dev_info->CaptureFormats_len =
        PTP_GetArray16((uint16_t *)&dev_info->CaptureFormats, data,
                       PTP_di_OperationsSupported + totallen);

    totallen = totallen + dev_info->CaptureFormats_len * sizeof(uint16_t) +
               sizeof(uint32_t);
    dev_info->ImageFormats_len =
        PTP_GetArray16((uint16_t *)&dev_info->ImageFormats, data,
                       PTP_di_OperationsSupported + totallen);

    totallen = totallen + dev_info->ImageFormats_len * sizeof(uint16_t) +
               sizeof(uint32_t);
    PTP_GetString(dev_info->Manufacturer,
                  &data[PTP_di_OperationsSupported + totallen], &len);

    totallen += len * 2 + 1;
    PTP_GetString(dev_info->Model, &data[PTP_di_OperationsSupported + totallen],
                  &len);

    totallen += len * 2 + 1;
    PTP_GetString(dev_info->DeviceVersion,
                  &data[PTP_di_OperationsSupported + totallen], &len);

    totallen += len * 2 + 1;
    PTP_GetString(dev_info->SerialNumber,
                  &data[PTP_di_OperationsSupported + totallen], &len);
  }
}
...
static void PTP_GetStorageInfo(USBH_HandleTypeDef *phost, uint32_t storage_id,
  ...
  PTP_GetString(stor_info->StorageDescription, &data[PTP_si_StorageDescription],
                &len);
  PTP_GetString(stor_info->VolumeLabel,
                &data[PTP_si_StorageDescription + len * 2 + 1], &len);
}
...
static void PTP_GetObjectPropDesc(USBH_HandleTypeDef *phost,
  ...
  PTP_GetDevicePropValue(phost, &offset, opdlen, &opd->FactoryDefaultValue,
                         opd->DataType);
  ...
  switch (opd->FormFlag) {
  case PTP_OPFF_Range:
    PTP_GetDevicePropValue(phost, &offset, opdlen,
                           &opd->FORM.Range.MinimumValue, opd->DataType);
    PTP_GetDevicePropValue(phost, &offset, opdlen,
                           &opd->FORM.Range.MaximumValue, opd->DataType);
    PTP_GetDevicePropValue(phost, &offset, opdlen, &opd->FORM.Range.StepSize,
                           opd->DataType);
    break;
  ...
  case PTP_OPFF_Enumeration:
    ...
    for (i = 0; i < opd->FORM.Enum.NumberOfValues; i++) {
      PTP_GetDevicePropValue(phost, &offset, opdlen,
                             &opd->FORM.Enum.SupportedValue[i], opd->DataType);
    }
    break;
  default:
    break;
...
static uint32_t PTP_GetObjectPropList(USBH_HandleTypeDef *phost,
  ...
  for (i = 0; i < prop_count; i++) {
    ...
    PTP_GetDevicePropValue(phost, &offset, len, &props[i].propval,
                           props[i].datatype);
...
static uint32_t PTP_GetArray16(uint16_t *array, uint8_t *data,
                               uint32_t offset) {
  uint32_t size, idx = 0;
...
static uint32_t PTP_GetArray32(uint32_t *array, uint8_t *data,
                               uint32_t offset) {
  uint32_t size, idx = 0;
 ...
 */
USBH_StatusTypeDef
USBH_PTP_GetDevicePropDesc(USBH_HandleTypeDef *phost, uint16_t propcode,
  ...
  case PTP_REQ_WAIT:
    ...
    if (status == USBH_OK) {
      devicepropertydesc->DevicePropertyCode =
          LE16(&data[PTP_dpd_DevicePropertyCode]);
 ...
 */
USBH_StatusTypeDef
USBH_PTP_GetStorageInfo(USBH_HandleTypeDef *phost, uint32_t storage_id,
 ...
 */
USBH_StatusTypeDef
USBH_PTP_GetObjectHandles(USBH_HandleTypeDef *phost, uint32_t storage_id,
  ...
  case PTP_REQ_WAIT:
    ...
    if (status == USBH_OK) {
      objecthandles->n =
          PTP_GetArray32(objecthandles->Handler,
                         MTP_Handle->ptp.data_container.payload.data, 0);
    }
    break;
...
USBH_StatusTypeDef USBH_PTP_GetObjectPropsSupported(USBH_HandleTypeDef *phost,
  ...
  case PTP_REQ_WAIT:
    ...
    if (status == USBH_OK) {
      *propnum =
          PTP_GetArray16(props, MTP_Handle->ptp.data_container.payload.data, 0);
    }
    break;
