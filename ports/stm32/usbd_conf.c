...
void HAL_PCD_MspInit(PCD_HandleTypeDef *hpcd) {
  if (hpcd->Instance == USB_OTG_FS) {
    ...
    mp_hal_pin_config(pin_A12, MP_HAL_PIN_MODE_ALT, MP_HAL_PIN_PULL_NONE,
                      otg_alt);
    mp_hal_pin_config_speed(pin_A12, GPIO_SPEED_FREQ_VERY_HIGH);

#if defined(MICROPY_HW_USB_VBUS_DETECT_PIN)
    // USB VBUS detect pin is always A9
    mp_hal_pin_config(MICROPY_HW_USB_VBUS_DETECT_PIN, MP_HAL_PIN_MODE_INPUT,
                      MP_HAL_PIN_PULL_NONE, 0);
#endif

#if defined(MICROPY_HW_USB_OTG_ID_PIN)
    // USB ID pin is always A10
    mp_hal_pin_config(MICROPY_HW_USB_OTG_ID_PIN, MP_HAL_PIN_MODE_ALT_OPEN_DRAIN,
                      MP_HAL_PIN_PULL_UP, otg_alt);
#endif
    ...
    mp_hal_pin_config(pin_B14, MP_HAL_PIN_MODE_ALT, MP_HAL_PIN_PULL_NONE,
                      otg_alt);
    mp_hal_pin_config_speed(pin_B14, GPIO_SPEED_FREQ_VERY_HIGH);
    mp_hal_pin_config(pin_B15, MP_HAL_PIN_MODE_ALT, MP_HAL_PIN_PULL_NONE,
                      otg_alt);
    ...
    static const mp_hal_pin_obj_t usb_pins[] = {
        ...
        MICROPY_HW_USB_HS_ULPI_DIR, // CLK, STP, NXT, DIR
        pin_A3,
        pin_B0,
        pin_B1,
        pin_B5,
    ...
    for (size_t i = 0; i < MP_ARRAY_SIZE(usb_pins); ++i) {
      mp_hal_pin_config(usb_pins[i], MP_HAL_PIN_MODE_ALT, MP_HAL_PIN_PULL_NONE,
                        GPIO_AF10_OTG_HS);
...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev, int high_speed,
  ...
  if (pdev->id == USB_PHY_FS_ID) {
    ...
    uint32_t fifo_offset =
        USBD_PMA_RESERVE; // need to reserve some data at start of FIFO
