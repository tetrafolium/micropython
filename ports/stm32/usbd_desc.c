...
void USBD_SetVIDPIDRelease(usbd_cdc_msc_hid_state_t *usbd, uint16_t vid,
                           uint16_t pid, uint16_t device_release_num,
                           int cdc_only) {
 ...
 */
STATIC uint8_t *USBD_DeviceDescriptor(USBD_HandleTypeDef *pdev,
                                      uint16_t *length) {
  uint8_t *dev_desc =
      ((usbd_cdc_msc_hid_state_t *)pdev->pClassData)->usbd_device_desc;
 ...
 */
STATIC uint8_t *USBD_StrDescriptor(USBD_HandleTypeDef *pdev, uint8_t idx,
                                   uint16_t *length) {
  char str_buf[16];
  const char *str = NULL;
  ...
  case USBD_IDX_SERIAL_STR: {
    ...
    snprintf(str_buf, sizeof(str_buf), "%02X%02X%02X%02X%02X%02X", id[11],
             id[10] + id[2], id[9], id[8] + id[0], id[7], id[6]);
  ...
  uint8_t *str_desc =
      ((usbd_cdc_msc_hid_state_t *)pdev->pClassData)->usbd_str_desc;
