...
// 512MBit external QSPI flash, used for either the filesystem or XIP memory
// mapped
...
#define MICROPY_HW_BDEV_IOCTL(op, arg)                                         \
  ((op) == BDEV_IOCTL_NUM_BLOCKS                                               \
       ? ((1 << MICROPY_HW_QSPIFLASH_SIZE_BITS_LOG2) / 8 / FLASH_BLOCK_SIZE)   \
       : (op) == BDEV_IOCTL_INIT                                               \
             ? spi_bdev_ioctl(&spi_bdev, (op), (uint32_t)&spiflash_config)     \
             : spi_bdev_ioctl(&spi_bdev, (op), (arg)))
#define MICROPY_HW_BDEV_READBLOCKS(dest, bl, n)                                \
  spi_bdev_readblocks(&spi_bdev, (dest), (bl), (n))
#define MICROPY_HW_BDEV_WRITEBLOCKS(src, bl, n)                                \
  spi_bdev_writeblocks(&spi_bdev, (src), (bl), (n))
#define MICROPY_HW_BDEV_SPIFLASH_EXTENDED                                      \
  (&spi_bdev) // for extended block protocol
#endif
...
#define MBOOT_SPIFLASH_ERASE_BLOCKS_PER_PAGE                                   \
  (128 / 4) // 128k page, 4k erase block
