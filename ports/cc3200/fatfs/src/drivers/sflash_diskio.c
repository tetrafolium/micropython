...
static bool sflash_access(_u32 mode,
                          _i32 (*sl_FsFunction)(_i32 FileHdl, _u32 Offset,
                                                _u8 *pData, _u32 Len)) {
  _i32 fileHandle;
  bool retval = false;
  ...
  if (0 == sl_FsOpen(sflash_block_name, mode, NULL, &fileHandle)) {
    if (SFLASH_BLOCK_SIZE ==
        sl_FsFunction(fileHandle, 0, sflash_block_cache, SFLASH_BLOCK_SIZE)) {
      retval = true;
    }
  ...
  if (!sflash_init_done) {
    ...
    for (int i = 0; i < SFLASH_BLOCK_COUNT; i++) {
      ...
      if (sl_FsGetInfo(sflash_block_name, 0, &FsFileInfo) != 0) {
        if (!sl_FsOpen(sflash_block_name,
                       FS_MODE_OPEN_CREATE(SFLASH_BLOCK_SIZE, 0), NULL,
                       &fileHandle)) {
          sl_FsClose(fileHandle, NULL, NULL, 0);
          sl_LockObjUnlock(&wlan_LockObj);
...
DRESULT sflash_disk_read(BYTE *buff, DWORD sector, UINT count) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    memcpy(buff, &sflash_block_cache[(secindex * SFLASH_SECTOR_SIZE)],
           SFLASH_SECTOR_SIZE);
...
DRESULT sflash_disk_write(const BYTE *buff, DWORD sector, UINT count) {
  ...
  do {
    ...
    memcpy(&sflash_block_cache[(secindex * SFLASH_SECTOR_SIZE)], buff,
           SFLASH_SECTOR_SIZE);
    buff += SFLASH_SECTOR_SIZE;
