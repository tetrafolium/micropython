...
typedef struct tskTaskControlBlock {
  ...
  StackType_t *pxEndOfStack; /*< Points to the end of the stack on architectures
 ...
 */
static void
prvInitialiseNewTask(TaskFunction_t pxTaskCode, const char *const pcName,
...
TaskHandle_t xTaskCreateStatic(
    ...
    StaticTask_t
        *const pxTaskBuffer) /*lint !e971 Unqualified char types are allowed for
                                strings and single characters only. */
{
  TCB_t *pxNewTCB;
  ...
  if ((pxTaskBuffer != NULL) && (puxStackBuffer != NULL)) {
    ...
    function - use them. */
    pxNewTCB =
        (TCB_t *)pxTaskBuffer; /*lint !e740 Unusual cast is ok as the structures
...
xTaskCreate(TaskFunction_t pxTaskCode, const char *const pcName,
            ...
            TaskHandle_t *const
                pxCreatedTask) /*lint !e971 Unqualified char types are allowed
...
static void prvInitialiseNewTask(
    ...
    const MemoryRegion_t
        *const xRegions) /*lint !e971 Unqualified char types are allowed for
                            strings and single characters only. */
{
        ...
        (StackType_t
             *)(((portPOINTER_SIZE_TYPE)pxTopOfStack) &
                (~((portPOINTER_SIZE_TYPE)
                       portBYTE_ALIGNMENT_MASK))); /*lint !e923 MISRA exception.
...
void vTaskStartScheduler(void) {
...
#else
  {
    ...
    xReturn =
        xTaskCreate(prvIdleTask, "IDLE", configMINIMAL_STACK_SIZE, (void *)NULL,
                    (tskIDLE_PRIORITY | portPRIVILEGE_BIT),
                    &xIdleTaskHandle); /*lint !e961 MISRA exception, justified
                                          as it is not a redundant explicit cast
...
#if (INCLUDE_vTaskDelete == 1)
  {
    ...
    while (uxDeletedTasksWaitingCleanUp > (UBaseType_t)0U) {
      ...
      if (xListIsEmpty == pdFALSE) {
        ...
        {
          pxTCB =
              (TCB_t *)listGET_OWNER_OF_HEAD_ENTRY((&xTasksWaitingTermination));
          (void)uxListRemove(&(pxTCB->xStateListItem));
          --uxCurrentNumberOfTasks;
          --uxDeletedTasksWaitingCleanUp;
        }
