...
typedef struct QueueDefinition {
  ...
  union /* Use of a union is an exception to the coding standard to ensure two
           mutually exclusive structure members don't appear simultaneously
           (wasting RAM). */
  {
...
QueueHandle_t xQueueGenericCreateStatic(const UBaseType_t uxQueueLength,
  ...
  but is already set. */
  pxNewQueue = (Queue_t *)
      pxStaticQueue; /*lint !e740 Unusual cast is ok as the structures are
...
QueueHandle_t xQueueCreateMutexStatic(const uint8_t ucQueueType,
  ...
  pxNewQueue = (Queue_t *)xQueueGenericCreateStatic(
...
BaseType_t xQueueGiveMutexRecursive(QueueHandle_t xMutex) {
  ...
  if (pxMutex->pxMutexHolder ==
      (void *)xTaskGetCurrentTaskHandle()) /*lint !e961 Not a redundant cast as
...
BaseType_t xQueueTakeMutexRecursive(QueueHandle_t xMutex,
  ...
  if (pxMutex->pxMutexHolder ==
      (void *)xTaskGetCurrentTaskHandle()) /*lint !e961 Cast is not redundant as
                                              TaskHandle_t is a typedef. */
  {
    (pxMutex->u.uxRecursiveCallCount)++;
    xReturn = pdPASS;
...
#if (configUSE_MUTEXES == 1)
          {
            if (pxQueue->uxQueueType == queueQUEUE_IS_MUTEX) {
              ...
              pxQueue->pxMutexHolder = (int8_t *)
                  pvTaskIncrementMutexHeldCount(); /*lint !e961 Cast is not
                                                      redundant as TaskHandle_t
                                                      is a typedef. */
            } else {
              mtCOVERAGE_TEST_MARKER();
            }
...
QueueSetMemberHandle_t xQueueSelectFromSet(QueueSetHandle_t xQueueSet,
  ...
  (void)xQueueGenericReceive((QueueHandle_t)xQueueSet, &xReturn, xTicksToWait,
                             pdFALSE); /*lint !e961 Casting from one typedef to
