...
#define PDB_CONFIG                                                             \
  (PDB_SC_TRGSEL(15) | PDB_SC_PDBEN | PDB_SC_PDBIE | PDB_SC_CONT |             \
   PDB_SC_PRESCALER(2) | PDB_SC_MULT(0))
...
#define clamp(v, min_val, max_val)                                             \
  ((v) < (min_val) ? (min_val) : (v) > (max_val) ? (max_val) : (v))

static float map_uint_to_float(uint x, uint in_min, uint in_max, float out_min,
                               float out_max) {
  return (float)(x - in_min) * (out_max - out_min) / (float)(in_max - in_min) +
         (float)out_min;
}

static uint map_float_to_uint(float x, float in_min, float in_max, uint out_min,
                              uint out_max) {
  return (int)((x - in_min) * (float)(out_max - out_min) / (in_max - in_min) +
               (float)out_min);
}
...
pin_error:
  mp_raise_msg_varg(MP_QSTR_ValueError, MP_ERROR_TEXT("pin %d does not exist"),
                    pin);
}
...
static mp_obj_t servo_obj_angle(int n_args, const mp_obj_t *args) {
  ...
  if (n_args == 1) {
    ...
    float angle = map_uint_to_float(servo_ticks[self->servo_id],
                                    ...
                                    usToTicks(self->max_usecs), 0.0, 180.0);
  ...
  servo_ticks[self->servo_id] =
      map_float_to_uint(angle, 0.0F, 180.0F, usToTicks(self->min_usecs),
...
static void servo_obj_print(void (*print)(void *env, const char *fmt, ...),
...
static MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(servo_obj_min_usecs_obj, 1, 2,
                                           servo_obj_min_usecs);
static MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(servo_obj_max_usecs_obj, 1, 2,
                                           servo_obj_max_usecs);
static MP_DEFINE_CONST_FUN_OBJ_1(servo_obj_attached_obj, servo_obj_attached);
static MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(servo_obj_angle_obj, 1, 2,
                                           servo_obj_angle);
static MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(servo_obj_usecs_obj, 1, 2,
                                           servo_obj_usecs);
