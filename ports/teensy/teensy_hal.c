...
mp_uint_t mp_hal_ticks_ms(void) { return millis(); }

void mp_hal_delay_ms(mp_uint_t ms) { delay(ms); }
...
int mp_hal_stdin_rx_chr(void) {
  for (;;) {
    ...
    if (usb_vcp_recv_byte(&c) != 0) {
      return c;
    } else if (MP_STATE_PORT(pyb_stdio_uart) != NULL &&
               uart_rx_any(MP_STATE_PORT(pyb_stdio_uart))) {
...
void mp_hal_stdout_tx_strn_cooked(const char *str, size_t len) {
  ...
  if (MP_STATE_PORT(pyb_stdio_uart) != NULL) {
    void uart_tx_strn_cooked(pyb_uart_obj_t * uart_obj, const char *str,
                             uint len);
...
void extinct_register_pin(const void *pin, uint32_t mode, int hard_irq,
