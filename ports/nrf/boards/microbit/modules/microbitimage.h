...
typedef struct _greyscale_t {
  ...
  uint8_t width;
  uint8_t
      byte_data[]; /* Static initializer for this will have to be C, not C++ */
} greyscale_t;
...
/* This is an internal method it is up to the caller to validate the inputs */
uint8_t imageGetPixelValue(microbit_image_obj_t *p_image_obj, mp_int_t x,
                           mp_int_t y);
#endif
...
mp_obj_t microbit_image_slice(microbit_image_obj_t *img, mp_int_t start,
                              mp_int_t width, mp_int_t stride);
/* ref exists so that we can pull a string out of an object and not have it
 * GC'ed while oterating over it */
mp_obj_t scrolling_string_image_iterable(const char *str, mp_uint_t len,
                                         mp_obj_t ref, bool monospace,
                                         bool repeat);

#define SMALL_IMAGE(p0, p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, p12,     \
                    p13, p14, p15, p16, p17, p18, p19, p20, p21, p22, p23,     \
                    p44)                                                       \
  {                                                                            \
    {&microbit_image_type}, 1, 0, 0, (p44), {                                  \
      (p0) | ((p1) << 1) | ((p2) << 2) | ((p3) << 3) | ((p4) << 4) |           \
          ((p5) << 5) | ((p6) << 6) | ((p7) << 7),                             \
          (p8) | ((p9) << 1) | ((p10) << 2) | ((p11) << 3) | ((p12) << 4) |    \
              ((p13) << 5) | ((p14) << 6) | ((p15) << 7),                      \
          (p16) | ((p17) << 1) | ((p18) << 2) | ((p19) << 3) | ((p20) << 4) |  \
              ((p21) << 5) | ((p22) << 6) | ((p23) << 7)                       \
    }                                                                          \
  }
