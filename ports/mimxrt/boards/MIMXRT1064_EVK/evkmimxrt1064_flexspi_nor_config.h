...
enum {
  ...
  kFlexSpiDeviceType_MCP_NOR_NAND =
      0x12, //!< Flash device is MCP device, A1 is Serial NOR, A2 is Serial NAND
  kFlexSpiDeviceType_MCP_NOR_RAM =
      0x13, //!< Flash deivce is MCP device, A1 is Serial NOR, A2 is Serial RAMs
};
...
typedef struct _FlexSPIConfig {
  ...
  flexspi_lut_seq_t deviceModeSeq; //!< [0x014-0x017] Device mode sequence info,
                                   //!< [7:0] - LUT sequence id, [15:8] - LUt
  ...
  uint32_t controllerMiscOption; //!< [0x040-0x043] Controller Misc Options, see
  ...
  uint8_t lutCustomSeqEnable; //!< [0x047-0x047] LUT customization Enable, it is
                              //!< required if the program/erase cannot
  ...
  uint32_t commandInterval;  //!< [0x074-0x077] CS deselect interval between two
                             //!< commands
  uint16_t dataValidTime[2]; //!< [0x078-0x07b] CLK edge to data valid time for
                             //!< PORT A and PORT B, in terms of 0.1ns
  uint16_t busyOffset;       //!< [0x07c-0x07d] Busy offset, valid value: 0-31
  uint16_t busyBitPolarity; //!< [0x07e-0x07f] Busy flag polarity, 0 - busy flag
                            //!< is 1 when flash device is busy, 1 -
...
#define NOR_CMD_LUT_SEQ_IDX_READSTATUS                                         \
  CMD_LUT_SEQ_IDX_READSTATUS //!< 1  Read Status LUT sequence id in lookupTable
                             //!< stored in config block
#define NOR_CMD_LUT_SEQ_IDX_READSTATUS_XPI                                     \
  2 //!< 2  Read status DPI/QPI/OPI sequence id in lookupTable stored in config
    //!< block
#define NOR_CMD_LUT_SEQ_IDX_WRITEENABLE                                        \
  CMD_LUT_SEQ_IDX_WRITEENABLE //!< 3  Write Enable sequence id in lookupTable
                              //!< stored in config block
#define NOR_CMD_LUT_SEQ_IDX_WRITEENABLE_XPI                                    \
  4 //!< 4  Write Enable DPI/QPI/OPI sequence id in lookupTable stored in config
    //!< block
