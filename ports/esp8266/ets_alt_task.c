...
void dump_task(int prio, volatile struct task_entry *t) {
  printf("q for task %d: queue: %p, get ptr: %d, put ptr: %d, qlen: %d\n", prio,
         t->queue, t->i_get, t->i_put, t->qlen);
}
...
bool ets_loop_iter(void) {
  ...
  // static unsigned cnt;
  bool progress = false;
  for (volatile struct task_entry *t = emu_tasks;
       t < &emu_tasks[MP_ARRAY_SIZE(emu_tasks)]; t++) {
    ...
    if (t->i_get != t->i_put) {
      ...
      //    t - emu_tasks + FIRST_PRIO, t->task, t->queue[t->i_get].sig,
      //    t->queue[t->i_get].par);
      int idx = t->i_get;
      if (t->i_put == -1) {
        t->i_put = t->i_get;
...
// Timer init func is in ROM, and calls ets_task by relative addr directly in
// ROM so, we have to re-init task using our handler
void ets_timer_init() {
