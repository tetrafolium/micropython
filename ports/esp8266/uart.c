...
static ringbuf_t uart_ringbuf = {uart_ringbuf_array, sizeof(uart_ringbuf_array),
                                 0, 0};
 ...
 *                UART0 used for data TX/RX, RX buffer size is 0x100, interrupt
 *enabled UART1 just used for debug output Parameters   : uart_no, use UART0 or
 *UART1 defined ahead Returns      : NONE
 *******************************************************************************/
static void ICACHE_FLASH_ATTR uart_config(uint8 uart_no) {
  ...
  WRITE_PERI_REG(UART_CONF0(uart_no),
                 UartDev.exist_parity | UartDev.parity |
 ...
 * Returns      : NONE
 *******************************************************************************/
static void ICACHE_FLASH_ATTR uart_os_write_char(char c) {
  if (uart_os == -1) {
    return;
  }
  if (c == '\n') {
...
void ICACHE_FLASH_ATTR uart_os_config(int uart) { uart_os = uart; }
...
static void uart0_rx_intr_handler(void *para) {
  ...
  read_chars:
    ...
    WRITE_PERI_REG(UART_INT_CLR(UART_REPL),
...
// reading. Returns true if something available, false if not.
...
int uart_tx_any_room(uint8 uart) {
  uint32_t fifo_cnt =
      READ_PERI_REG(UART_STATUS(uart)) & (UART_TXFIFO_CNT << UART_TXFIFO_CNT_S);
  if ((fifo_cnt >> UART_TXFIFO_CNT_S & UART_TXFIFO_CNT) >= 126) {
    return false;
  }
  return true;
...
int uart_rx_char(void) { return ringbuf_get(&uart_ringbuf); }
...
int ICACHE_FLASH_ATTR uart0_get_rxbuf_len(void) { return uart_ringbuf.size; }
