...
STATIC void ICACHE_FLASH_ATTR pwm_insert_sort(struct pwm_single_param pwm[],
                                              uint8 n) {
  uint8 i;
...
void ICACHE_FLASH_ATTR pwm_start(void) {
  ...
  PWM_DBG("pwm_out_io_num[0]:%d,[1]:%d,[2]:%d\n", pwm_out_io_num[0],
          pwm_out_io_num[1], pwm_out_io_num[2]);
  ...
  PWM_DBG("i:%d period:%d ht:%d\n", pwm_channel_num, pwm.period,
          local_single[pwm_channel_num].h_time);
  ...
  *local_channel = pwm_channel_num + 1;
  PWM_DBG("1channel:%d,single[0]:%d,[1]:%d,[2]:%d,[3]:%d\n", *local_channel,
          local_single[0].h_time, local_single[1].h_time,
          local_single[2].h_time, local_single[3].h_time);
  ...
  for (i = pwm_channel_num; i > 0; i--) {
    if (local_single[i].h_time == local_single[i - 1].h_time) {
      ...
      for (j = i + 1; j < *local_channel; j++) {
        memcpy(&local_single[j - 1], &local_single[j],
  ...
  if (local_single[0].h_time == 0) {
    ...
    for (i = 1; i < *local_channel; i++) {
      memcpy(&local_single[i - 1], &local_single[i],
             sizeof(struct pwm_single_param));
    }
  ...
  if (pwm_timer_down == 1) {
    ...
    // start
    gpio_output_set(local_single[0].gpio_set, local_single[0].gpio_clear,
                    pwm_gpio, 0);
 ...
 * Returns      : NONE
 *******************************************************************************/
void ICACHE_FLASH_ATTR pwm_set_duty(int16_t duty, uint8 channel) {
  uint8 i;
  for (i = 0; i < pwm_channel_num; i++) {
 ...
 * Returns      : NONE
 *******************************************************************************/
void ICACHE_FLASH_ATTR pwm_set_freq(uint16 freq, uint8 channel) {
  LOCK_PWM(critical); // enter critical
 ...
 * Returns      : NONE
 *******************************************************************************/
uint16 ICACHE_FLASH_ATTR pwm_get_duty(uint8 channel) {
  uint8 i;
  for (i = 0; i < pwm_channel_num; i++) {
    if (pwm_out_io_num[i] == channel) {
      channel = i;
 ...
 *******************************************************************************/
uint16 ICACHE_FLASH_ATTR pwm_get_freq(uint8 channel) { return pwm.freq; }
 ...
 * Returns      : NONE
 *******************************************************************************/
STATIC void ICACHE_RAM_ATTR pwm_tim1_intr_handler(void *dummy) {
  (void)dummy;

  RTC_CLR_REG_MASK(FRC1_INT_ADDRESS, FRC1_INT_CLR_MASK);

  if (pwm_current_channel >=
      (*pwm_channel - 1)) { // *pwm_channel may change outside
    ...
    gpio_output_set(pwm_single[*pwm_channel - 1].gpio_set,
                    pwm_single[*pwm_channel - 1].gpio_clear, pwm_gpio, 0);
    ...
    gpio_output_set(pwm_single[pwm_current_channel].gpio_set,
                    pwm_single[pwm_current_channel].gpio_clear, pwm_gpio, 0);
...
int ICACHE_FLASH_ATTR pwm_add(uint8_t pin_id, uint32_t pin_mux,
                              uint32_t pin_func) {
  PWM_DBG("--Function pwm_add() is called. channel:%d\n", channel);
  PWM_DBG("pwm_gpio:%x,pwm_channel_num:%d\n", pwm_gpio, pwm_channel_num);
  PWM_DBG("pwm_out_io_num[0]:%d,[1]:%d,[2]:%d\n", pwm_out_io_num[0],
          pwm_out_io_num[1], pwm_out_io_num[2]);
  PWM_DBG("pwm.duty[0]:%d,[1]:%d,[2]:%d\n", pwm.duty[0], pwm.duty[1],
          pwm.duty[2]);
  int channel = -1;
  ...
  for (i = 0; i < PWM_CHANNEL; i++) {
    ...
    if (pwm_out_io_num[i] == -1) { // empty exist
      ...
      GPIO_REG_WRITE(
          GPIO_PIN_ADDR(GPIO_ID_PIN(pin_num[channel])),
...
bool ICACHE_FLASH_ATTR pwm_delete(uint8 channel) {
