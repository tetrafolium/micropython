...
static int lfs1_cache_read(lfs1_t *lfs1, lfs1_cache_t *rcache,
  ...
  while (size > 0) {
    if (pcache && block == pcache->block && off >= pcache->off &&
      ...
      lfs1_size_t diff =
          lfs1_min(size, lfs1->cfg->prog_size - (off - pcache->off));
    ...
    if (block == rcache->block && off >= rcache->off &&
      ...
      lfs1_size_t diff =
          lfs1_min(size, lfs1->cfg->read_size - (off - rcache->off));
    ...
    int err = lfs1->cfg->read(lfs1->cfg, rcache->block, rcache->off,
                              rcache->buffer, lfs1->cfg->read_size);
    if (err) {
      return err;
    }
  }
...
static int lfs1_cache_cmp(lfs1_t *lfs1, lfs1_cache_t *rcache,
                          const lfs1_cache_t *pcache, lfs1_block_t block,
                          lfs1_off_t off, const void *buffer,
                          lfs1_size_t size) {
  const uint8_t *data = buffer;

  for (lfs1_off_t i = 0; i < size; i++) {
    uint8_t c;
    int err = lfs1_cache_read(lfs1, rcache, pcache, block, off + i, &c, 1);
    if (err) {
      return err;
    }
...
static int lfs1_cache_flush(lfs1_t *lfs1, lfs1_cache_t *pcache,
                            lfs1_cache_t *rcache) {
  if (pcache->block != 0xffffffff) {
    int err = lfs1->cfg->prog(lfs1->cfg, pcache->block, pcache->off,
                              pcache->buffer, lfs1->cfg->prog_size);
    if (err) {
      return err;
    }

    if (rcache) {
      int res = lfs1_cache_cmp(lfs1, rcache, NULL, pcache->block, pcache->off,
                               pcache->buffer, lfs1->cfg->prog_size);
      if (res < 0) {
        return res;
      }
...
static int lfs1_cache_prog(lfs1_t *lfs1, lfs1_cache_t *pcache,
  ...
  while (size > 0) {
    ...
    if (off % lfs1->cfg->prog_size == 0 && size >= lfs1->cfg->prog_size) {
      // bypass pcache?
      ...
      if (rcache) {
        int res = lfs1_cache_cmp(lfs1, rcache, NULL, block, off, data, diff);
        if (res < 0) {
          return res;
        }
...
static int lfs1_bd_read(lfs1_t *lfs1, lfs1_block_t block, lfs1_off_t off,
                        void *buffer, lfs1_size_t size) {
  // if we ever do more than writes to alternating pairs,
  // this may need to consider pcache
  return lfs1_cache_read(lfs1, &lfs1->rcache, NULL, block, off, buffer, size);
}

static int lfs1_bd_prog(lfs1_t *lfs1, lfs1_block_t block, lfs1_off_t off,
                        const void *buffer, lfs1_size_t size) {
  return lfs1_cache_prog(lfs1, &lfs1->pcache, NULL, block, off, buffer, size);
}

static int lfs1_bd_cmp(lfs1_t *lfs1, lfs1_block_t block, lfs1_off_t off,
                       const void *buffer, lfs1_size_t size) {
  return lfs1_cache_cmp(lfs1, &lfs1->rcache, NULL, block, off, buffer, size);
}

static int lfs1_bd_crc(lfs1_t *lfs1, lfs1_block_t block, lfs1_off_t off,
                       lfs1_size_t size, uint32_t *crc) {
...
static int lfs1_relocate(lfs1_t *lfs1, const lfs1_block_t oldpair[2],
                         const lfs1_block_t newpair[2]);
...
static int lfs1_alloc_lookahead(void *p, lfs1_block_t block) {
  ...
  lfs1_block_t off = ((block - lfs1->free.off) + lfs1->cfg->block_count) %
...
static int lfs1_alloc(lfs1_t *lfs1, lfs1_block_t *block) {
  while (true) {
    ...
    if (lfs1->free.ack == 0) {
      LFS1_WARN("No more free space %" PRIu32, lfs1->free.i + lfs1->free.off);
      return LFS1_ERR_NOSPC;
    }

    lfs1->free.off =
        (lfs1->free.off + lfs1->free.size) % lfs1->cfg->block_count;
...
static inline int lfs1_paircmp(const lfs1_block_t paira[2],
                               const lfs1_block_t pairb[2]) {
  return !(paira[0] == pairb[0] || paira[1] == pairb[1] ||
           paira[0] == pairb[1] || paira[1] == pairb[0]);
}

static inline bool lfs1_pairsync(const lfs1_block_t paira[2],
                                 const lfs1_block_t pairb[2]) {
  return (paira[0] == pairb[0] && paira[1] == pairb[1]) ||
         (paira[0] == pairb[1] && paira[1] == pairb[0]);
}
...
static int lfs1_dir_fetch(lfs1_t *lfs1, lfs1_dir_t *dir,
                          const lfs1_block_t pair[2]) {
  ...
  if (!valid) {
    LFS1_ERROR("Corrupted dir pair at %" PRIu32 " %" PRIu32, tpair[0],
               tpair[1]);
    return LFS1_ERR_CORRUPT;
  }
...
static int lfs1_dir_commit(lfs1_t *lfs1, lfs1_dir_t *dir,
    ...
    if (true) {
      ...
      while (newoff < (0x7fffffff & dir->d.size) - 4) {
        if (i < count && regions[i].oldoff == oldoff) {
          ...
          err = lfs1_bd_prog(lfs1, dir->pair[0], newoff, regions[i].newdata,
                             regions[i].newlen);
          if (err) {
            if (err == LFS1_ERR_CORRUPT) {
              goto relocate;
      ...
      err = lfs1_bd_crc(lfs1, dir->pair[0], 0, (0x7fffffff & dir->d.size) - 4,
                        &ncrc);
      if (err) {
        return err;
      }
  ...
  relocate:
    ...
    if (lfs1_paircmp(oldpair, (const lfs1_block_t[2]){0, 1}) == 0) {
      LFS1_WARN("Superblock %" PRIu32 " has become unwritable", oldpair[0]);
      return LFS1_ERR_CORRUPT;
    }
...
static int lfs1_dir_update(lfs1_t *lfs1, lfs1_dir_t *dir, lfs1_entry_t *entry,
                           const void *data) {
  lfs1_entry_tole32(&entry->d);
  int err = lfs1_dir_commit(
      lfs1, dir,
      (struct lfs1_region[]){
...
static int lfs1_dir_append(lfs1_t *lfs1, lfs1_dir_t *dir, lfs1_entry_t *entry,
                           const void *data) {
  ...
  while (true) {
    if (dir->d.size + lfs1_entry_size(entry) <= lfs1->cfg->block_size) {
      ...
      int err = lfs1_dir_commit(
          lfs1, dir,
          (struct lfs1_region[]){{entry->off, 0, &entry->d, sizeof(entry->d)},
...
static int lfs1_dir_remove(lfs1_t *lfs1, lfs1_dir_t *dir, lfs1_entry_t *entry) {
  ...
  if ((dir->d.size & 0x7fffffff) ==
      sizeof(dir->d) + 4 + lfs1_entry_size(entry)) {
    lfs1_dir_t pdir;
  ...
  // shift out the entry
  int err = lfs1_dir_commit(lfs1, dir,
                            (struct lfs1_region[]){
...
static int lfs1_dir_next(lfs1_t *lfs1, lfs1_dir_t *dir, lfs1_entry_t *entry) {
  ...
  int err =
      lfs1_bd_read(lfs1, dir->pair[0], dir->off, &entry->d, sizeof(entry->d));
  lfs1_entry_fromle32(&entry->d);
  if (err) {
    return err;
  }
...
static int lfs1_dir_find(lfs1_t *lfs1, lfs1_dir_t *dir, lfs1_entry_t *entry,
                         const char **path) {
...
static int lfs1_ctz_find(lfs1_t *lfs1, lfs1_cache_t *rcache,
                         const lfs1_cache_t *pcache, lfs1_block_t head,
                         lfs1_size_t size, lfs1_size_t pos, lfs1_block_t *block,
                         lfs1_off_t *off) {
  if (size == 0) {
    *block = 0xffffffff;
    *off = 0;
  ...
  while (current > target) {
    lfs1_size_t skip =
        lfs1_min(lfs1_npw2(current - target + 1) - 1, lfs1_ctz(current));
...
static int lfs1_ctz_extend(lfs1_t *lfs1, lfs1_cache_t *rcache,
                           lfs1_cache_t *pcache, lfs1_block_t head,
                           lfs1_size_t size, lfs1_block_t *block,
                           lfs1_off_t *off) {
  while (true) {
    ...
    if (true) {
      ...
      if (size != lfs1->cfg->block_size) {
        for (lfs1_off_t i = 0; i < size; i++) {
          uint8_t data;
          err = lfs1_cache_read(lfs1, rcache, NULL, head, i, &data, 1);
          if (err) {
            return err;
          }

          err = lfs1_cache_prog(lfs1, pcache, rcache, nblock, i, &data, 1);
          if (err) {
            if (err == LFS1_ERR_CORRUPT) {
              goto relocate;
            }
      ...
      for (lfs1_off_t i = 0; i < skips; i++) {
        ...
        err = lfs1_cache_prog(lfs1, pcache, rcache, nblock, 4 * i, &head, 4);
        head = lfs1_fromle32(head);
        if (err) {
          if (err == LFS1_ERR_CORRUPT) {
            goto relocate;
        ...
        if (i != skips - 1) {
          err = lfs1_cache_read(lfs1, rcache, NULL, head, 4 * i, &head, 4);
          head = lfs1_fromle32(head);
          if (err) {
            return err;
          }
...
static int lfs1_ctz_traverse(lfs1_t *lfs1, lfs1_cache_t *rcache,
                             const lfs1_cache_t *pcache, lfs1_block_t head,
                             lfs1_size_t size, int (*cb)(void *, lfs1_block_t),
                             void *data) {
  if (size == 0) {
    return 0;
  }
...
int lfs1_file_opencfg(lfs1_t *lfs1, lfs1_file_t *file, const char *path,
                      int flags, const struct lfs1_file_config *cfg) {
  // deorphan if we haven't yet, needed at most once after poweron
...
int lfs1_file_open(lfs1_t *lfs1, lfs1_file_t *file, const char *path,
                   int flags) {
...
relocate:
  ...
  for (lfs1_off_t i = 0; i < file->off; i++) {
    uint8_t data;
    err = lfs1_cache_read(lfs1, &lfs1->rcache, &file->cache, file->block, i,
                          &data, 1);
    if (err) {
      return err;
    }

    err = lfs1_cache_prog(lfs1, &lfs1->pcache, &lfs1->rcache, nblock, i, &data,
                          1);
    if (err) {
      if (err == LFS1_ERR_CORRUPT) {
        goto relocate;
...
int lfs1_file_sync(lfs1_t *lfs1, lfs1_file_t *file) {
  ...
  if ((file->flags & LFS1_F_DIRTY) && !(file->flags & LFS1_F_ERRED) &&
    ...
    err = lfs1_bd_read(lfs1, cwd.pair[0], entry.off, &entry.d, sizeof(entry.d));
    lfs1_entry_fromle32(&entry.d);
    if (err) {
      return err;
    }
...
lfs1_ssize_t lfs1_file_read(lfs1_t *lfs1, lfs1_file_t *file, void *buffer,
                            lfs1_size_t size) {
  uint8_t *data = buffer;
  ...
  while (nsize > 0) {
    ...
    if (!(file->flags & LFS1_F_READING) || file->off == lfs1->cfg->block_size) {
      int err = lfs1_ctz_find(lfs1, &file->cache, NULL, file->head, file->size,
    ...
    int err = lfs1_cache_read(lfs1, &file->cache, NULL, file->block, file->off,
                              data, diff);
    if (err) {
      return err;
    }
...
lfs1_ssize_t lfs1_file_write(lfs1_t *lfs1, lfs1_file_t *file,
  ...
  while (nsize > 0) {
    // check if we need a new block
    if (!(file->flags & LFS1_F_WRITING) || file->off == lfs1->cfg->block_size) {
      if (!(file->flags & LFS1_F_WRITING) && file->pos > 0) {
        // find out which block we're extending from
        int err =
            lfs1_ctz_find(lfs1, &file->cache, NULL, file->head, file->size,
      ...
      lfs1_alloc_ack(lfs1);
      int err = lfs1_ctz_extend(lfs1, &lfs1->rcache, &file->cache, file->block,
                                file->pos, &file->block, &file->off);
      if (err) {
        file->flags |= LFS1_F_ERRED;
        return err;
    ...
    lfs1_size_t diff = lfs1_min(nsize, lfs1->cfg->block_size - file->off);
    while (true) {
      int err = lfs1_cache_prog(lfs1, &file->cache, &lfs1->rcache, file->block,
                                file->off, data, diff);
      if (err) {
        if (err == LFS1_ERR_CORRUPT) {
          goto relocate;
...
lfs1_soff_t lfs1_file_seek(lfs1_t *lfs1, lfs1_file_t *file, lfs1_soff_t off,
                           int whence) {
...
int lfs1_file_truncate(lfs1_t *lfs1, lfs1_file_t *file, lfs1_off_t size) {
  ...
  if (size < oldsize) {
    ...
    err = lfs1_ctz_find(lfs1, &file->cache, NULL, file->head, file->size, size,
                        &file->head, &(lfs1_off_t){0});
    if (err) {
      return err;
    }
...
int lfs1_format(lfs1_t *lfs1, const struct lfs1_config *cfg) {
  ...
  if (true) {
    ...
    for (int i = 0; i < 2; i++) {
      err = lfs1_dir_commit(
          lfs1, &superdir,
          (struct lfs1_region[]){{sizeof(superdir.d), sizeof(superblock.d),
...
int lfs1_mount(lfs1_t *lfs1, const struct lfs1_config *cfg) {
  ...
  if (true) {
    ...
    if (!err) {
      err = lfs1_bd_read(lfs1, dir.pair[0], sizeof(dir.d), &superblock.d,
                         sizeof(superblock.d));
...
int lfs1_traverse(lfs1_t *lfs1, int (*cb)(void *, lfs1_block_t), void *data) {
  ...
  while (true) {
    ...
    while (dir.off + sizeof(entry.d) <= (0x7fffffff & dir.d.size) - 4) {
      err = lfs1_bd_read(lfs1, dir.pair[0], dir.off, &entry.d, sizeof(entry.d));
      lfs1_entry_fromle32(&entry.d);
      if (err) {
        return err;
      }
      ...
      if ((0x70 & entry.d.type) == (0x70 & LFS1_TYPE_REG)) {
        err = lfs1_ctz_traverse(lfs1, &lfs1->rcache, NULL, entry.d.u.file.head,
  ...
  for (lfs1_file_t *f = lfs1->files; f; f = f->next) {
    if (f->flags & LFS1_F_DIRTY) {
      int err = lfs1_ctz_traverse(lfs1, &lfs1->rcache, &f->cache, f->head,
                                  f->size, cb, data);
      if (err) {
        return err;
      }
    ...
    if (f->flags & LFS1_F_WRITING) {
      int err = lfs1_ctz_traverse(lfs1, &lfs1->rcache, &f->cache, f->block,
                                  f->pos, cb, data);
      if (err) {
        return err;
      }
...
static int lfs1_pred(lfs1_t *lfs1, const lfs1_block_t dir[2],
                     lfs1_dir_t *pdir) {
  if (lfs1_pairisnull(lfs1->root)) {
    return 0;
  }
...
static int lfs1_relocate(lfs1_t *lfs1, const lfs1_block_t oldpair[2],
  ...
  if (res) {
    ...
    if (lfs1_paircmp(oldpair, lfs1->root) == 0) {
      LFS1_DEBUG("Relocating root %" PRIu32 " %" PRIu32, newpair[0],
                 newpair[1]);
...
int lfs1_deorphan(lfs1_t *lfs1) {
  ...
  for (lfs1_size_t i = 0; i < lfs1->cfg->block_count; i++) {
    ...
    if (!(0x80000000 & pdir.d.size)) {
      ...
      if (!res) {
        // we are an orphan
        LFS1_DEBUG("Found orphan %" PRIu32 " %" PRIu32, pdir.d.tail[0],
                   pdir.d.tail[1]);
      ...
      if (!lfs1_pairsync(entry.d.u.dir, pdir.d.tail)) {
        // we have desynced
        LFS1_DEBUG("Found desync %" PRIu32 " %" PRIu32, entry.d.u.dir[0],
                   entry.d.u.dir[1]);
    ...
    while (true) {
      ...
      if (entry.d.type & 0x80) {
        ...
        if (moved) {
          LFS1_DEBUG("Found move %" PRIu32 " %" PRIu32, entry.d.u.dir[0],
                     entry.d.u.dir[1]);
          err = lfs1_dir_remove(lfs1, &cwd, &entry);
          if (err) {
            return err;
