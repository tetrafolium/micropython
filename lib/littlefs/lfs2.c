...
static int lfs2_bd_read(lfs2_t *lfs2, const lfs2_cache_t *pcache,
                        lfs2_cache_t *rcache, lfs2_size_t hint,
                        lfs2_block_t block, lfs2_off_t off, void *buffer,
                        lfs2_size_t size) {
  uint8_t *data = buffer;
  if (block >= lfs2->cfg->block_count || off + size > lfs2->cfg->block_size) {
  ...
  while (size > 0) {
    ...
    rcache->size =
        lfs2_min(lfs2_min(lfs2_alignup(off + hint, lfs2->cfg->read_size),
                          lfs2->cfg->block_size) -
                     rcache->off,
                 lfs2->cfg->cache_size);
    int err = lfs2->cfg->read(lfs2->cfg, rcache->block, rcache->off,
...
static int lfs2_bd_cmp(lfs2_t *lfs2, const lfs2_cache_t *pcache,
                       lfs2_cache_t *rcache, lfs2_size_t hint,
                       lfs2_block_t block, lfs2_off_t off, const void *buffer,
                       lfs2_size_t size) {
  ...
  for (lfs2_off_t i = 0; i < size; i += diff) {
    ...
    int res = lfs2_bd_read(lfs2, pcache, rcache, hint - i, block, off + i, &dat,
                           diff);
    if (res) {
      return res;
    }
...
static int lfs2_bd_flush(lfs2_t *lfs2, lfs2_cache_t *pcache,
                         lfs2_cache_t *rcache, bool validate) {
  if (pcache->block != LFS2_BLOCK_NULL && pcache->block != LFS2_BLOCK_INLINE) {
    ...
    int err = lfs2->cfg->prog(lfs2->cfg, pcache->block, pcache->off,
                              pcache->buffer, diff);
    LFS2_ASSERT(err <= 0);
    if (err) {
      return err;
    ...
    if (validate) {
      ...
      int res = lfs2_bd_cmp(lfs2, NULL, rcache, diff, pcache->block,
...
#ifndef LFS2_READONLY
static int lfs2_bd_sync(lfs2_t *lfs2, lfs2_cache_t *pcache,
                        lfs2_cache_t *rcache, bool validate) {
  lfs2_cache_drop(lfs2, rcache);
...
#ifndef LFS2_READONLY
static int lfs2_bd_prog(lfs2_t *lfs2, lfs2_cache_t *pcache,
                        lfs2_cache_t *rcache, bool validate, lfs2_block_t block,
  ...
  while (size > 0) {
    if (block == pcache->block && off >= pcache->off &&
      ...
      lfs2_size_t diff =
          lfs2_min(size, lfs2->cfg->cache_size - (off - pcache->off));
...
static inline int lfs2_pair_cmp(const lfs2_block_t paira[2],
                                const lfs2_block_t pairb[2]) {
  return !(paira[0] == pairb[0] || paira[1] == pairb[1] ||
           paira[0] == pairb[1] || paira[1] == pairb[0]);
}

static inline bool lfs2_pair_sync(const lfs2_block_t paira[2],
                                  const lfs2_block_t pairb[2]) {
  return (paira[0] == pairb[0] && paira[1] == pairb[1]) ||
         (paira[0] == pairb[1] && paira[1] == pairb[0]);
}
...
static int lfs2_dir_compact(lfs2_t *lfs2, lfs2_mdir_t *dir,
                            const struct lfs2_mattr *attrs, int attrcount,
...
static void lfs2_fs_prepmove(lfs2_t *lfs2, uint16_t id,
...
static lfs2_stag_t lfs2_fs_parent(lfs2_t *lfs2, const lfs2_block_t dir[2],
                                  lfs2_mdir_t *parent);
static int lfs2_fs_relocate(lfs2_t *lfs2, const lfs2_block_t oldpair[2],
                            lfs2_block_t newpair[2]);
static int lfs2_fs_forceconsistency(lfs2_t *lfs2);
#endif

#ifdef LFS2_MIGRATE
static int lfs21_traverse(lfs2_t *lfs2, int (*cb)(void *, lfs2_block_t),
                          void *data);
#endif
...
static int lfs2_fs_rawtraverse(lfs2_t *lfs2,
                               int (*cb)(void *data, lfs2_block_t block),
...
static int lfs2_alloc_lookahead(void *p, lfs2_block_t block) {
  ...
  lfs2_block_t off = ((block - lfs2->free.off) + lfs2->cfg->block_count) %
...
static int lfs2_alloc(lfs2_t *lfs2, lfs2_block_t *block) {
  while (true) {
    ...
    if (lfs2->free.ack == 0) {
      LFS2_ERROR("No more free space %" PRIu32, lfs2->free.i + lfs2->free.off);
      return LFS2_ERR_NOSPC;
    }

    lfs2->free.off =
        (lfs2->free.off + lfs2->free.size) % lfs2->cfg->block_count;
...
static lfs2_stag_t lfs2_dir_getslice(lfs2_t *lfs2, const lfs2_mdir_t *dir,
  ...
  while (off >= sizeof(lfs2_tag_t) + lfs2_tag_dsize(ntag)) {
    ...
    lfs2_tag_t tag = ntag;
    int err = lfs2_bd_read(lfs2, NULL, &lfs2->rcache, sizeof(ntag),
    ...
    if (lfs2_tag_id(gmask) != 0 && lfs2_tag_type1(tag) == LFS2_TYPE_SPLICE &&
    ...
    if ((gmask & tag) == (gmask & (gtag - gdiff))) {
      ...
      err = lfs2_bd_read(lfs2, NULL, &lfs2->rcache, diff, dir->pair[0],
...
static int lfs2_dir_getread(lfs2_t *lfs2, const lfs2_mdir_t *dir,
  ...
  while (size > 0) {
    ...
    rcache->size = lfs2_min(lfs2_alignup(off + hint, lfs2->cfg->read_size),
                            lfs2->cfg->cache_size);
    int err = lfs2_dir_getslice(lfs2, dir, gmask, gtag, rcache->off,
                                rcache->buffer, rcache->size);
    if (err < 0) {
      return err;
    }
...
#ifndef LFS2_READONLY
static int lfs2_dir_traverse_filter(void *p, lfs2_tag_t tag,
                                    const void *buffer) {
  lfs2_tag_t *filtertag = p;
  (void)buffer;

  // which mask depends on unique bit in tag structure
  uint32_t mask = (tag & LFS2_MKTAG(0x100, 0, 0)) ? LFS2_MKTAG(0x7ff, 0x3ff, 0)
                                                  : LFS2_MKTAG(0x700, 0x3ff, 0);

  // check for redundancy
  if ((mask & tag) == (mask & *filtertag) || lfs2_tag_isdelete(*filtertag) ||
...
#ifndef LFS2_READONLY
static int lfs2_dir_traverse(
    lfs2_t *lfs2, const lfs2_mdir_t *dir, lfs2_off_t off, lfs2_tag_t ptag,
    const struct lfs2_mattr *attrs, int attrcount, lfs2_tag_t tmask,
    lfs2_tag_t ttag, uint16_t begin, uint16_t end, int16_t diff,
  ...
  while (true) {
    ...
    if (off + lfs2_tag_dsize(ptag) < dir->off) {
      ...
      int err = lfs2_bd_read(lfs2, NULL, &lfs2->rcache, sizeof(tag),
    ...
    if (lfs2_tag_id(tmask) != 0) {
      ...
      int filter =
          lfs2_dir_traverse(lfs2, dir, off, ptag, attrs, attrcount, 0, 0, 0, 0,
                            0, lfs2_dir_traverse_filter, &tag);
      if (filter < 0) {
        return filter;
      }
    ...
    if (lfs2_tag_type3(tag) == LFS2_FROM_NOOP) {
      ...
      int err = lfs2_dir_traverse(lfs2, buffer, 0, 0xffffffff, NULL, 0,
                                  LFS2_MKTAG(0x600, 0x3ff, 0),
                                  LFS2_MKTAG(LFS2_TYPE_STRUCT, 0, 0), fromid,
                                  fromid + 1, toid - fromid + diff, cb, data);
      if (err) {
        return err;
      }
      ...
      for (unsigned i = 0; i < lfs2_tag_size(tag); i++) {
        ...
        int err = cb(data,
                     LFS2_MKTAG(LFS2_TYPE_USERATTR + a[i].type,
                                lfs2_tag_id(tag) + diff, a[i].size),
                     a[i].buffer);
        if (err) {
          return err;
        }
...
static lfs2_stag_t
lfs2_dir_fetchmatch(lfs2_t *lfs2, lfs2_mdir_t *dir, const lfs2_block_t pair[2],
  ...
  int r = 0;
  for (int i = 0; i < 2; i++) {
    int err = lfs2_bd_read(lfs2, NULL, &lfs2->rcache, sizeof(revs[i]), pair[i],
    ...
    if (err != LFS2_ERR_CORRUPT && lfs2_scmp(revs[i], revs[(i + 1) % 2]) > 0) {
      r = i;
    }
  }
  ...
  for (int i = 0; i < 2; i++) {
    ...
    while (true) {
      ...
      lfs2_tag_t tag;
      off += lfs2_tag_dsize(ptag);
      int err = lfs2_bd_read(lfs2, NULL, &lfs2->rcache, lfs2->cfg->block_size,
                             dir->pair[0], off, &tag, sizeof(tag));
      if (err) {
      ...
      if (lfs2_tag_type1(tag) == LFS2_TYPE_CRC) {
        ...
        uint32_t dcrc;
        err =
            lfs2_bd_read(lfs2, NULL, &lfs2->rcache, lfs2->cfg->block_size,
      ...
      for (lfs2_off_t j = sizeof(tag); j < lfs2_tag_dsize(tag); j++) {
        uint8_t dat;
        err = lfs2_bd_read(lfs2, NULL, &lfs2->rcache, lfs2->cfg->block_size,
      ...
      if ((fmask & tag) == (fmask & ftag)) {
        int res = cb(data, tag,
                     &(struct lfs2_diskoff){dir->pair[0], off + sizeof(tag)});
        if (res < 0) {
...
static int lfs2_dir_fetch(lfs2_t *lfs2, lfs2_mdir_t *dir,
                          const lfs2_block_t pair[2]) {
  // note, mask=-1, tag=-1 can never match a tag since this
  // pattern has the invalid bit set
  return (int)lfs2_dir_fetchmatch(lfs2, dir, pair, (lfs2_tag_t)-1,
                                  (lfs2_tag_t)-1, NULL, NULL, NULL);
}
...
static int lfs2_dir_getgstate(lfs2_t *lfs2, const lfs2_mdir_t *dir,
  ...
  lfs2_gstate_t temp;
  lfs2_stag_t res =
      lfs2_dir_get(lfs2, dir, LFS2_MKTAG(0x7ff, 0, 0),
...
static int lfs2_dir_getinfo(lfs2_t *lfs2, lfs2_mdir_t *dir, uint16_t id,
  ...
  lfs2_stag_t tag = lfs2_dir_get(
      lfs2, dir, LFS2_MKTAG(0x780, 0x3ff, 0),
...
static int lfs2_dir_find_match(void *data, lfs2_tag_t tag, const void *buffer) {
  ...
  int res = lfs2_bd_cmp(lfs2, NULL, &lfs2->rcache, diff, disk->block, disk->off,
                        name->name, diff);
  if (res != LFS2_CMP_EQ) {
    return res;
  }
  ...
  nextname:
    ...
    *path = name;
    ...
    if (lfs2_tag_id(tag) != 0x3ff) {
      lfs2_stag_t res = lfs2_dir_get(
          lfs2, dir, LFS2_MKTAG(0x700, 0x3ff, 0),
    ...
    while (true) {
      tag = lfs2_dir_fetchmatch(
          lfs2, dir, dir->tail, LFS2_MKTAG(0x780, 0, 0),
          LFS2_MKTAG(LFS2_TYPE_NAME, 0, namelen),
          // are we last name?
          (strchr(name, '/') == NULL) ? id : NULL, lfs2_dir_find_match,
          &(struct lfs2_dir_find_match){lfs2, name, namelen});
      if (tag < 0) {
        return tag;
      }
...
static int lfs2_dir_commitprog(lfs2_t *lfs2, struct lfs2_commit *commit,
                               const void *buffer, lfs2_size_t size) {
  int err =
      lfs2_bd_prog(lfs2, &lfs2->pcache, &lfs2->rcache, false, commit->block,
...
static int lfs2_dir_commitattr(lfs2_t *lfs2, struct lfs2_commit *commit,
  ...
  if (!(tag & 0x80000000)) {
    ...
    for (lfs2_off_t i = 0; i < dsize - sizeof(tag); i++) {
      ...
      uint8_t dat;
      err = lfs2_bd_read(lfs2, NULL, &lfs2->rcache, dsize - sizeof(tag) - i,
...
static int lfs2_dir_commitcrc(lfs2_t *lfs2, struct lfs2_commit *commit) {
  ...
  const lfs2_off_t end =
      lfs2_alignup(commit->off + 2 * sizeof(uint32_t), lfs2->cfg->prog_size);
  ...
  while (commit->off < end) {
    ...
    err = lfs2_bd_prog(lfs2, &lfs2->pcache, &lfs2->rcache, false, commit->block,
  ...
  while (off < end) {
    ...
    for (lfs2_off_t i = off; i < noff + sizeof(uint32_t); i++) {
      ...
      uint8_t dat;
      err = lfs2_bd_read(lfs2, NULL, &lfs2->rcache, noff + sizeof(uint32_t) - i,
...
static int lfs2_dir_alloc(lfs2_t *lfs2, lfs2_mdir_t *dir) {
  ...
  int err = lfs2_bd_read(lfs2, NULL, &lfs2->rcache, sizeof(dir->rev),
...
#ifndef LFS2_READONLY
static int lfs2_dir_split(lfs2_t *lfs2, lfs2_mdir_t *dir,
                          const struct lfs2_mattr *attrs, int attrcount,
...
static int lfs2_dir_compact(lfs2_t *lfs2, lfs2_mdir_t *dir,
  ...
  while (end - begin > 1) {
    // find size
    lfs2_size_t size = 0;
    int err = lfs2_dir_traverse(lfs2, source, 0, 0xffffffff, attrs, attrcount,
                                LFS2_MKTAG(0x400, 0x3ff, 0),
                                LFS2_MKTAG(LFS2_TYPE_NAME, 0, 0), begin, end,
                                -begin, lfs2_dir_commit_size, &size);
    if (err) {
      return err;
    }
    ...
    err =
        lfs2_dir_split(lfs2, dir, attrs, attrcount, source, begin + split, end);
    if (err) {
  ...
  if (lfs2->cfg->block_cycles > 0 &&
    ...
    if (lfs2_pair_cmp(dir->pair, (const lfs2_block_t[2]){0, 1}) == 0) {
      ...
      if ((lfs2_size_t)res < lfs2->cfg->block_count / 2) {
        ...
        int err =
            lfs2_dir_split(lfs2, dir, attrs, attrcount, source, begin, end);
  ...
  while (true) {
    {
      ...
      // write out header
      dir->rev = lfs2_tole32(dir->rev);
      err = lfs2_dir_commitprog(lfs2, &commit, &dir->rev, sizeof(dir->rev));
      dir->rev = lfs2_fromle32(dir->rev);
      if (err) {
        if (err == LFS2_ERR_CORRUPT) {
          goto relocate;
      ...
      err = lfs2_dir_traverse(lfs2, source, 0, 0xffffffff, attrs, attrcount,
                              LFS2_MKTAG(0x400, 0x3ff, 0),
                              LFS2_MKTAG(LFS2_TYPE_NAME, 0, 0), begin, end,
                              -begin, lfs2_dir_commit_commit,
                              &(struct lfs2_dir_commit_commit){lfs2, &commit});
      if (err) {
        if (err == LFS2_ERR_CORRUPT) {
          goto relocate;
        }
      ...
      if (!lfs2_pair_isnull(dir->tail)) {
        ...
        err = lfs2_dir_commitattr(
            lfs2, &commit, LFS2_MKTAG(LFS2_TYPE_TAIL + dir->split, 0x3ff, 8),
            dir->tail);
        lfs2_pair_fromle32(dir->tail);
        if (err) {
          if (err == LFS2_ERR_CORRUPT) {
      ...
      if (!lfs2_gstate_iszero(&delta)) {
        ...
        err = lfs2_dir_commitattr(
            lfs2, &commit,
            LFS2_MKTAG(LFS2_TYPE_MOVESTATE, 0x3ff, sizeof(delta)), &delta);
        if (err) {
          if (err == LFS2_ERR_CORRUPT) {
            goto relocate;
          }
  ...
  relocate:
    ...
    if (lfs2_pair_cmp(dir->pair, (const lfs2_block_t[2]){0, 1}) == 0) {
      LFS2_WARN("Superblock 0x%" PRIx32 " has become unwritable", dir->pair[1]);
      return LFS2_ERR_NOSPC;
    }
...
static int lfs2_dir_commit(lfs2_t *lfs2, lfs2_mdir_t *dir,
  ...
  if (dir->erased || dir->count >= 0xff) {
    ...
    lfs2_pair_tole32(dir->tail);
    int err =
        lfs2_dir_traverse(lfs2, dir, dir->off, dir->etag, attrs, attrcount, 0,
                          0, 0, 0, 0, lfs2_dir_commit_commit,
                          &(struct lfs2_dir_commit_commit){lfs2, &commit});
    lfs2_pair_fromle32(dir->tail);
    if (err) {
    ...
    if (!lfs2_gstate_iszero(&delta)) {
      ...
      lfs2_gstate_tole32(&delta);
      err = lfs2_dir_commitattr(
          lfs2, &commit, LFS2_MKTAG(LFS2_TYPE_MOVESTATE, 0x3ff, sizeof(delta)),
          &delta);
      if (err) {
        if (err == LFS2_ERR_NOSPC || err == LFS2_ERR_CORRUPT) {
          goto compact;
  ...
  compact:
    ...
    int err = lfs2_dir_compact(lfs2, dir, attrs, attrcount, dir, 0, dir->count);
    if (err) {
      *dir = olddir;
      return err;
    }
...
static int lfs2_rawmkdir(lfs2_t *lfs2, const char *path) {
  ...
  err = lfs2_dir_commit(
      lfs2, &dir,
      LFS2_MKATTRS({LFS2_MKTAG(LFS2_TYPE_SOFTTAIL, 0x3ff, 8), pred.tail}));
  ...
  if (cwd.m.split) {
    ...
    err = lfs2_dir_commit(
        lfs2, &pred,
        LFS2_MKATTRS({LFS2_MKTAG(LFS2_TYPE_SOFTTAIL, 0x3ff, 8), dir.pair}));
  ...
  err = lfs2_dir_commit(
      lfs2, &cwd.m,
      LFS2_MKATTRS({LFS2_MKTAG(LFS2_TYPE_CREATE, id, 0), NULL},
                   {LFS2_MKTAG(LFS2_TYPE_DIR, id, nlen), path},
                   {LFS2_MKTAG(LFS2_TYPE_DIRSTRUCT, id, 8), dir.pair},
                   {LFS2_MKTAG_IF(!cwd.m.split, LFS2_TYPE_SOFTTAIL, 0x3ff, 8),
                    dir.pair}));
  lfs2_pair_fromle32(dir.pair);
  if (err) {
    return err;
...
static int lfs2_dir_rawopen(lfs2_t *lfs2, lfs2_dir_t *dir, const char *path) {
  ...
  if (lfs2_tag_id(tag) == 0x3ff) {
    ...
    lfs2_stag_t res =
        lfs2_dir_get(lfs2, &dir->m, LFS2_MKTAG(0x700, 0x3ff, 0),
...
static int lfs2_dir_rawread(lfs2_t *lfs2, lfs2_dir_t *dir,
                            struct lfs2_info *info) {
...
static int lfs2_ctz_find(lfs2_t *lfs2, const lfs2_cache_t *pcache,
                         lfs2_cache_t *rcache, lfs2_block_t head,
                         lfs2_size_t size, lfs2_size_t pos, lfs2_block_t *block,
                         lfs2_off_t *off) {
  if (size == 0) {
  ...
  while (current > target) {
    lfs2_size_t skip =
        lfs2_min(lfs2_npw2(current - target + 1) - 1, lfs2_ctz(current));

    int err = lfs2_bd_read(lfs2, pcache, rcache, sizeof(head), head, 4 * skip,
                           &head, sizeof(head));
...
#ifndef LFS2_READONLY
static int lfs2_ctz_extend(lfs2_t *lfs2, lfs2_cache_t *pcache,
                           lfs2_cache_t *rcache, lfs2_block_t head,
                           lfs2_size_t size, lfs2_block_t *block,
                           lfs2_off_t *off) {
  while (true) {
...

    {
      ...
      if (noff != lfs2->cfg->block_size) {
        for (lfs2_off_t i = 0; i < noff; i++) {
          uint8_t data;
          err = lfs2_bd_read(lfs2, NULL, rcache, noff - i, head, i, &data, 1);
          if (err) {
            return err;
          }

          err = lfs2_bd_prog(lfs2, pcache, rcache, true, nblock, i, &data, 1);
          if (err) {
            if (err == LFS2_ERR_CORRUPT) {
              goto relocate;
            }
      ...
      for (lfs2_off_t i = 0; i < skips; i++) {
        ...
        err =
            lfs2_bd_prog(lfs2, pcache, rcache, true, nblock, 4 * i, &nhead, 4);
        ...
        if (i != skips - 1) {
          err = lfs2_bd_read(lfs2, NULL, rcache, sizeof(nhead), nhead, 4 * i,
                             &nhead, sizeof(nhead));
          nhead = lfs2_fromle32(nhead);
          if (err) {
            return err;
...
static int lfs2_ctz_traverse(lfs2_t *lfs2, const lfs2_cache_t *pcache,
                             lfs2_cache_t *rcache, lfs2_block_t head,
                             lfs2_size_t size, int (*cb)(void *, lfs2_block_t),
                             void *data) {
  if (size == 0) {
    return 0;
  }
  ...
  while (true) {
    ...
    int count = 2 - (index & 1);
    err = lfs2_bd_read(lfs2, pcache, rcache, count * sizeof(head), head, 0,
...
static int lfs2_file_rawopencfg(lfs2_t *lfs2, lfs2_file_t *file,
  ...
  if (tag == LFS2_ERR_NOENT) {
    ...
    err = lfs2_dir_commit(
        lfs2, &file->m,
        LFS2_MKATTRS({LFS2_MKTAG(LFS2_TYPE_CREATE, file->id, 0), NULL},
  ...
  for (unsigned i = 0; i < file->cfg->attr_count; i++) {
    ...
    if ((file->flags & LFS2_O_RDONLY) == LFS2_O_RDONLY) {
      lfs2_stag_t res =
          lfs2_dir_get(lfs2, &file->m, LFS2_MKTAG(0x7ff, 0x3ff, 0),
  ...
  if (lfs2_tag_type3(tag) == LFS2_TYPE_INLINESTRUCT) {
    ...
    if (file->ctz.size > 0) {
      lfs2_stag_t res =
          lfs2_dir_get(lfs2, &file->m, LFS2_MKTAG(0x700, 0x3ff, 0),
...
static int lfs2_file_rawopen(lfs2_t *lfs2, lfs2_file_t *file, const char *path,
                             int flags) {
  ...
  while (true) {
    ...
    for (lfs2_off_t i = 0; i < file->off; i++) {
      ...
      if (file->flags & LFS2_F_INLINE) {
        err = lfs2_dir_getread(
            lfs2, &file->m,
            // note we evict inline files before they can be dirty
            NULL, &file->cache, file->off - i, LFS2_MKTAG(0xfff, 0x1ff, 0),
            LFS2_MKTAG(LFS2_TYPE_INLINESTRUCT, file->id, 0), i, &data, 1);
        if (err) {
          return err;
        }
      } else {
        err = lfs2_bd_read(lfs2, &file->cache, &lfs2->rcache, file->off - i,
                           file->block, i, &data, 1);
        if (err) {
          return err;
        }
      ...
      err = lfs2_bd_prog(lfs2, &lfs2->pcache, &lfs2->rcache, true, nblock, i,
                         &data, 1);
      if (err) {
        if (err == LFS2_ERR_CORRUPT) {
          goto relocate;
...
static int lfs2_file_rawsync(lfs2_t *lfs2, lfs2_file_t *file) {
  ...
  if ((file->flags & LFS2_F_DIRTY) && !lfs2_pair_isnull(file->m.pair)) {
    // update dir entry
    uint16_t type;
    const void *buffer;
    lfs2_size_t size;
    ...
    err =
        lfs2_dir_commit(lfs2, &file->m,
                        LFS2_MKATTRS({LFS2_MKTAG(type, file->id, size), buffer},
...
static lfs2_ssize_t lfs2_file_rawread(lfs2_t *lfs2, lfs2_file_t *file,
  ...
  while (nsize > 0) {
    ...
    if (!(file->flags & LFS2_F_READING) || file->off == lfs2->cfg->block_size) {
      if (!(file->flags & LFS2_F_INLINE)) {
        int err =
            lfs2_ctz_find(lfs2, NULL, &file->cache, file->ctz.head,
    ...
    if (file->flags & LFS2_F_INLINE) {
      int err =
          lfs2_dir_getread(lfs2, &file->m, NULL, &file->cache,
                           lfs2->cfg->block_size, LFS2_MKTAG(0xfff, 0x1ff, 0),
      ...
      int err = lfs2_bd_read(lfs2, NULL, &file->cache, lfs2->cfg->block_size,
...
static lfs2_ssize_t lfs2_file_rawwrite(lfs2_t *lfs2, lfs2_file_t *file,
  ...
  if ((file->flags & LFS2_F_INLINE) &&
      lfs2_max(file->pos + nsize, file->ctz.size) >
          lfs2_min(0x3fe, lfs2_min(lfs2->cfg->cache_size,
  ...
  while (nsize > 0) {
    // check if we need a new block
    if (!(file->flags & LFS2_F_WRITING) || file->off == lfs2->cfg->block_size) {
      if (!(file->flags & LFS2_F_INLINE)) {
        if (!(file->flags & LFS2_F_WRITING) && file->pos > 0) {
          // find out which block we're extending from
          int err = lfs2_ctz_find(lfs2, NULL, &file->cache, file->ctz.head,
        ...
        lfs2_alloc_ack(lfs2);
        int err =
            lfs2_ctz_extend(lfs2, &file->cache, &lfs2->rcache, file->block,
...
#ifndef LFS2_READONLY
static int lfs2_file_rawtruncate(lfs2_t *lfs2, lfs2_file_t *file,
                                 lfs2_off_t size) {
  LFS2_ASSERT((file->flags & LFS2_O_WRONLY) == LFS2_O_WRONLY);
  ...
  if (size < oldsize) {
    ...
    err = lfs2_ctz_find(lfs2, NULL, &file->cache, file->ctz.head,
...
static int lfs2_rawstat(lfs2_t *lfs2, const char *path,
                        struct lfs2_info *info) {
  lfs2_mdir_t cwd;
...
static int lfs2_rawremove(lfs2_t *lfs2, const char *path) {
  ...
  if (lfs2_tag_type3(tag) == LFS2_TYPE_DIR) {
    ...
    lfs2_block_t pair[2];
    lfs2_stag_t res =
        lfs2_dir_get(lfs2, &cwd, LFS2_MKTAG(0x700, 0x3ff, 0),
                     LFS2_MKTAG(LFS2_TYPE_STRUCT, lfs2_tag_id(tag), 8), pair);
    if (res < 0) {
      return (int)res;
    }
  ...
  err = lfs2_dir_commit(
      lfs2, &cwd,
      LFS2_MKATTRS({LFS2_MKTAG(LFS2_TYPE_DELETE, lfs2_tag_id(tag), 0), NULL}));
  if (err) {
    lfs2->mlist = dir.next;
    return err;
  }
...
#ifndef LFS2_READONLY
static int lfs2_rawrename(lfs2_t *lfs2, const char *oldpath,
                          const char *newpath) {
  // deorphan if we haven't yet, needed at most once after poweron
  int err = lfs2_fs_forceconsistency(lfs2);
  if (err) {
  ...
  if (prevtag == LFS2_ERR_NOENT) {
    ...
    lfs2_stag_t res =
        lfs2_dir_get(lfs2, &newcwd, LFS2_MKTAG(0x700, 0x3ff, 0),
  ...
  err = lfs2_dir_commit(
      lfs2, &newcwd,
      LFS2_MKATTRS(
          {LFS2_MKTAG_IF(prevtag != LFS2_ERR_NOENT, LFS2_TYPE_DELETE, newid, 0),
           NULL},
          ...
          {LFS2_MKTAG_IF(samepair, LFS2_TYPE_DELETE, newoldid, 0), NULL}));
  if (err) {
...
static lfs2_ssize_t lfs2_rawgetattr(lfs2_t *lfs2, const char *path,
  ...
  tag = lfs2_dir_get(
      lfs2, &cwd, LFS2_MKTAG(0x7ff, 0x3ff, 0),
      LFS2_MKTAG(LFS2_TYPE_USERATTR + type, id, lfs2_min(size, lfs2->attr_max)),
      buffer);
  if (tag < 0) {
    if (tag == LFS2_ERR_NOENT) {
...
#ifndef LFS2_READONLY
static int lfs2_commitattr(lfs2_t *lfs2, const char *path, uint8_t type,
                           const void *buffer, lfs2_size_t size) {
  lfs2_mdir_t cwd;
  lfs2_stag_t tag = lfs2_dir_find(lfs2, &cwd, &path, NULL);
  if (tag < 0) {
  ...
  return lfs2_dir_commit(
      lfs2, &cwd,
      LFS2_MKATTRS({LFS2_MKTAG(LFS2_TYPE_USERATTR + type, id, size), buffer}));
}
#endif

#ifndef LFS2_READONLY
static int lfs2_rawsetattr(lfs2_t *lfs2, const char *path, uint8_t type,
                           const void *buffer, lfs2_size_t size) {
...
static int lfs2_rawformat(lfs2_t *lfs2, const struct lfs2_config *cfg) {
  int err = 0;
  {
    ...
    lfs2->free.off = 0;
    lfs2->free.size =
        lfs2_min(8 * lfs2->cfg->lookahead_size, lfs2->cfg->block_count);
    lfs2->free.i = 0;
    lfs2_alloc_ack(lfs2);
    ...
    err = lfs2_dir_commit(
        lfs2, &root,
        LFS2_MKATTRS({LFS2_MKTAG(LFS2_TYPE_CREATE, 0, 0), NULL},
...
static int lfs2_rawmount(lfs2_t *lfs2, const struct lfs2_config *cfg) {
  ...
  while (!lfs2_pair_isnull(dir.tail)) {
    ...
    lfs2_stag_t tag = lfs2_dir_fetchmatch(
        lfs2, &dir, dir.tail, LFS2_MKTAG(0x7ff, 0x3ff, 0),
        LFS2_MKTAG(LFS2_TYPE_SUPERBLOCK, 0, 8), NULL, lfs2_dir_find_match,
        &(struct lfs2_dir_find_match){lfs2, "littlefs", 8});
    if (tag < 0) {
      err = tag;
      goto cleanup;
    }
    ...
    if (tag && !lfs2_tag_isdelete(tag)) {
      ...
      lfs2_superblock_t superblock;
      tag = lfs2_dir_get(
          lfs2, &dir, LFS2_MKTAG(0x7ff, 0x3ff, 0),
          LFS2_MKTAG(LFS2_TYPE_INLINESTRUCT, 0, sizeof(superblock)),
          &superblock);
      if (tag < 0) {
        err = tag;
      ...
      if ((major_version != LFS2_DISK_VERSION_MAJOR ||
        ...
        LFS2_ERROR("Invalid version v%" PRIu16 ".%" PRIu16, major_version,
                   minor_version);
        err = LFS2_ERR_INVAL;
        goto cleanup;
      }
  ...
  if (!lfs2_gstate_iszero(&lfs2->gstate)) {
    LFS2_DEBUG("Found pending gstate 0x%08" PRIx32 "%08" PRIx32 "%08" PRIx32,
               lfs2->gstate.tag, lfs2->gstate.pair[0], lfs2->gstate.pair[1]);
  }
...
static int lfs2_rawunmount(lfs2_t *lfs2) { return lfs2_deinit(lfs2); }

/// Filesystem filesystem operations ///
int lfs2_fs_rawtraverse(lfs2_t *lfs2, int (*cb)(void *data, lfs2_block_t block),
                        void *data, bool includeorphans) {
  ...
  while (!lfs2_pair_isnull(dir.tail)) {
    ...
    for (uint16_t id = 0; id < dir.count; id++) {
      struct lfs2_ctz ctz;
      lfs2_stag_t tag =
          lfs2_dir_get(lfs2, &dir, LFS2_MKTAG(0x700, 0x3ff, 0),
                       LFS2_MKTAG(LFS2_TYPE_STRUCT, id, sizeof(ctz)), &ctz);
      if (tag < 0) {
        if (tag == LFS2_ERR_NOENT) {
          continue;
      ...
      if (lfs2_tag_type3(tag) == LFS2_TYPE_CTZSTRUCT) {
        err = lfs2_ctz_traverse(lfs2, NULL, &lfs2->rcache, ctz.head, ctz.size,
                                cb, data);
        if (err) {
          return err;
        }
      } else if (includeorphans && lfs2_tag_type3(tag) == LFS2_TYPE_DIRSTRUCT) {
        for (int i = 0; i < 2; i++) {
  ...
  for (lfs2_file_t *f = (lfs2_file_t *)lfs2->mlist; f; f = f->next) {
    ...
    if ((f->flags & LFS2_F_DIRTY) && !(f->flags & LFS2_F_INLINE)) {
      int err = lfs2_ctz_traverse(lfs2, &f->cache, &lfs2->rcache, f->ctz.head,
                                  f->ctz.size, cb, data);
      if (err) {
        return err;
      }
    ...
    if ((f->flags & LFS2_F_WRITING) && !(f->flags & LFS2_F_INLINE)) {
      int err = lfs2_ctz_traverse(lfs2, &f->cache, &lfs2->rcache, f->block,
                                  f->pos, cb, data);
      if (err) {
        return err;
      }
...
#ifndef LFS2_READONLY
static int lfs2_fs_pred(lfs2_t *lfs2, const lfs2_block_t pair[2],
                        lfs2_mdir_t *pdir) {
...
#ifndef LFS2_READONLY
static int lfs2_fs_parent_match(void *data, lfs2_tag_t tag,
                                const void *buffer) {
  ...
  int err =
      lfs2_bd_read(lfs2, &lfs2->pcache, &lfs2->rcache, lfs2->cfg->block_size,
...
static lfs2_stag_t lfs2_fs_parent(lfs2_t *lfs2, const lfs2_block_t pair[2],
  ...
  while (!lfs2_pair_isnull(parent->tail)) {
    ...
    lfs2_stag_t tag = lfs2_dir_fetchmatch(
        lfs2, parent, parent->tail, LFS2_MKTAG(0x7ff, 0, 0x3ff),
        LFS2_MKTAG(LFS2_TYPE_DIRSTRUCT, 0, 8), NULL, lfs2_fs_parent_match,
        &(struct lfs2_fs_parent_match){lfs2, {pair[0], pair[1]}});
...
static int lfs2_fs_relocate(lfs2_t *lfs2, const lfs2_block_t oldpair[2],
  ...
  if (tag != LFS2_ERR_NOENT) {
    ...
    int err = lfs2_dir_commit(
        lfs2, &parent,
        LFS2_MKATTRS(
            {LFS2_MKTAG_IF(moveid != 0x3ff, LFS2_TYPE_DELETE, moveid, 0), NULL},
            {tag, newpair}));
...
static void lfs2_fs_prepmove(lfs2_t *lfs2, uint16_t id,
                             const lfs2_block_t pair[2]) {
  lfs2->gstate.tag =
      ((lfs2->gstate.tag & ~LFS2_MKTAG(0x7ff, 0x3ff, 0)) |
...
static int lfs2_fs_demove(lfs2_t *lfs2) {
  ...
  LFS2_DEBUG("Fixing move {0x%" PRIx32 ", 0x%" PRIx32 "} 0x%" PRIx16,
             lfs2->gdisk.pair[0], lfs2->gdisk.pair[1],
  ...
  err = lfs2_dir_commit(
      lfs2, &movedir,
      LFS2_MKATTRS({LFS2_MKTAG(LFS2_TYPE_DELETE, moveid, 0), NULL}));
  if (err) {
    return err;
  }
...
static int lfs2_fs_deorphan(lfs2_t *lfs2) {
  ...
  while (!lfs2_pair_isnull(pdir.tail)) {
    ...
    if (!pdir.split) {
      ...
      if (tag == LFS2_ERR_NOENT) {
        // we are an orphan
        LFS2_DEBUG("Fixing orphan {0x%" PRIx32 ", 0x%" PRIx32 "}", pdir.tail[0],
                   pdir.tail[1]);
      ...
      lfs2_block_t pair[2];
      lfs2_stag_t res =
          lfs2_dir_get(lfs2, &parent, LFS2_MKTAG(0x7ff, 0x3ff, 0), tag, pair);
      if (res < 0) {
        return res;
      }
      lfs2_pair_fromle32(pair);
      ...
      if (!lfs2_pair_sync(pair, pdir.tail)) {
        ...
        err = lfs2_dir_commit(
            lfs2, &pdir,
            LFS2_MKATTRS({LFS2_MKTAG(LFS2_TYPE_SOFTTAIL, 0x3ff, 8), pair}));
        lfs2_pair_fromle32(pair);
        if (err) {
          return err;
        }
...
static int lfs21_bd_read(lfs2_t *lfs2, lfs2_block_t block, lfs2_off_t off,
                         void *buffer, lfs2_size_t size) {
  // if we ever do more than writes to alternating pairs,
  // this may need to consider pcache
  return lfs2_bd_read(lfs2, &lfs2->pcache, &lfs2->rcache, size, block, off,
                      buffer, size);
}

static int lfs21_bd_crc(lfs2_t *lfs2, lfs2_block_t block, lfs2_off_t off,
                        lfs2_size_t size, uint32_t *crc) {
...
static int lfs21_dir_fetch(lfs2_t *lfs2, lfs21_dir_t *dir,
                           const lfs2_block_t pair[2]) {
  ...
  if (!valid) {
    LFS2_ERROR("Corrupted dir pair at {0x%" PRIx32 ", 0x%" PRIx32 "}", tpair[0],
               tpair[1]);
    return LFS2_ERR_CORRUPT;
  }
...
static int lfs21_dir_next(lfs2_t *lfs2, lfs21_dir_t *dir,
                          lfs21_entry_t *entry) {
  ...
  int err =
      lfs21_bd_read(lfs2, dir->pair[0], dir->off, &entry->d, sizeof(entry->d));
  ...
  while (true) {
    ...
    while (dir.off + sizeof(entry.d) <= (0x7fffffff & dir.d.size) - 4) {
      ...
      if ((0x70 & entry.d.type) == (0x70 & LFS21_TYPE_REG)) {
        err = lfs2_ctz_traverse(lfs2, NULL, &lfs2->rcache, entry.d.u.file.head,
...
static int lfs21_mount(lfs2_t *lfs2, struct lfs21 *lfs21,
  ...
  int err = 0;
  {
    ...
    if (!err) {
      err = lfs21_bd_read(lfs2, dir.pair[0], sizeof(dir.d), &superblock.d,
                          sizeof(superblock.d));
    ...
    if (err || memcmp(superblock.d.magic, "littlefs", 8) != 0) {
      LFS2_ERROR("Invalid superblock at {0x%" PRIx32 ", 0x%" PRIx32 "}", 0, 1);
      err = LFS2_ERR_CORRUPT;
      goto cleanup;
    }
...
static int lfs21_unmount(lfs2_t *lfs2) { return lfs2_deinit(lfs2); }
...

  {
    ...
    while (!lfs2_pair_isnull(dir1.d.tail)) {
      ...
      while (true) {
        ...
        err = lfs2_dir_commit(
            ...
            LFS2_MKATTRS(
                ...
                {LFS2_MKTAG_IF_ELSE(isdir, LFS2_TYPE_DIR, id, entry1.d.nlen,
                                    LFS2_TYPE_REG, id, entry1.d.nlen),
                 name},
                {LFS2_MKTAG_IF_ELSE(isdir, LFS2_TYPE_DIRSTRUCT, id,
                                    sizeof(entry1.d.u), LFS2_TYPE_CTZSTRUCT, id,
                                    sizeof(entry1.d.u)),
                 &entry1.d.u}));
      ...
      for (lfs2_off_t i = 0; i < dir2.off; i++) {
        uint8_t dat;
        err = lfs2_bd_read(lfs2, NULL, &lfs2->rcache, dir2.off, dir2.pair[0], i,
                           &dat, 1);
        if (err) {
          goto cleanup;
        }

        err = lfs2_bd_prog(lfs2, &lfs2->pcache, &lfs2->rcache, true,
                           dir1.head[1], i, &dat, 1);
        if (err) {
          goto cleanup;
        }
    ...
    err = lfs2_dir_commit(
        lfs2, &dir2,
        LFS2_MKATTRS({LFS2_MKTAG(LFS2_TYPE_CREATE, 0, 0)},
                     {LFS2_MKTAG(LFS2_TYPE_SUPERBLOCK, 0, 8), "littlefs"},
                     {LFS2_MKTAG(LFS2_TYPE_INLINESTRUCT, 0, sizeof(superblock)),
                      &superblock}));
    if (err) {
...
#ifndef LFS2_READONLY
int lfs2_setattr(lfs2_t *lfs2, const char *path, uint8_t type,
                 const void *buffer, lfs2_size_t size) {
  int err = LFS2_LOCK(lfs2->cfg);
  if (err) {
    return err;
...
int lfs2_removeattr(lfs2_t *lfs2, const char *path, uint8_t type) {
  ...
  LFS2_TRACE("lfs2_removeattr(%p, \"%s\", %" PRIu8 ")", (void *)lfs2, path,
             type);
...
int lfs2_file_open(lfs2_t *lfs2, lfs2_file_t *file, const char *path,
                   int flags) {
  int err = LFS2_LOCK(lfs2->cfg);
  if (err) {
    return err;
  }
  LFS2_TRACE("lfs2_file_open(%p, %p, \"%s\", %x)", (void *)lfs2, (void *)file,
             path, flags);
...
int lfs2_file_opencfg(lfs2_t *lfs2, lfs2_file_t *file, const char *path,
                      int flags, const struct lfs2_file_config *cfg) {
  int err = LFS2_LOCK(lfs2->cfg);
  if (err) {
    return err;
  }
  LFS2_TRACE("lfs2_file_opencfg(%p, %p, \"%s\", %x, %p {"
             ...
             (void *)lfs2, (void *)file, path, flags, (void *)cfg, cfg->buffer,
...
lfs2_ssize_t lfs2_file_read(lfs2_t *lfs2, lfs2_file_t *file, void *buffer,
                            lfs2_size_t size) {
  int err = LFS2_LOCK(lfs2->cfg);
  if (err) {
    return err;
  }
  LFS2_TRACE("lfs2_file_read(%p, %p, %p, %" PRIu32 ")", (void *)lfs2,
...
lfs2_ssize_t lfs2_file_write(lfs2_t *lfs2, lfs2_file_t *file,
  ...
  LFS2_TRACE("lfs2_file_write(%p, %p, %p, %" PRIu32 ")", (void *)lfs2,
             (void *)file, buffer, size);
  LFS2_ASSERT(lfs2_mlist_isopen(lfs2->mlist, (struct lfs2_mlist *)file));
...
lfs2_soff_t lfs2_file_seek(lfs2_t *lfs2, lfs2_file_t *file, lfs2_soff_t off,
                           int whence) {
  int err = LFS2_LOCK(lfs2->cfg);
  if (err) {
    return err;
  }
  LFS2_TRACE("lfs2_file_seek(%p, %p, %" PRId32 ", %d)", (void *)lfs2,
             (void *)file, off, whence);
  LFS2_ASSERT(lfs2_mlist_isopen(lfs2->mlist, (struct lfs2_mlist *)file));
...
int lfs2_file_truncate(lfs2_t *lfs2, lfs2_file_t *file, lfs2_off_t size) {
  ...
  LFS2_TRACE("lfs2_file_truncate(%p, %p, %" PRIu32 ")", (void *)lfs2,
             (void *)file, size);
  LFS2_ASSERT(lfs2_mlist_isopen(lfs2->mlist, (struct lfs2_mlist *)file));
...
int lfs2_dir_read(lfs2_t *lfs2, lfs2_dir_t *dir, struct lfs2_info *info) {
  ...
  LFS2_TRACE("lfs2_dir_read(%p, %p, %p)", (void *)lfs2, (void *)dir,
             (void *)info);
...
int lfs2_dir_seek(lfs2_t *lfs2, lfs2_dir_t *dir, lfs2_off_t off) {
  ...
  LFS2_TRACE("lfs2_dir_seek(%p, %p, %" PRIu32 ")", (void *)lfs2, (void *)dir,
             off);
...
int lfs2_fs_traverse(lfs2_t *lfs2, int (*cb)(void *, lfs2_block_t),
  ...
  LFS2_TRACE("lfs2_fs_traverse(%p, %p, %p)", (void *)lfs2,
             (void *)(uintptr_t)cb, data);
