...
// test_code checks that the provided code results in the provided error string
// for any size buffer. It calls mbedtls_strerror() to fill a buffer that is
// from 1 to 100 bytes in length and then checks that the buffer contents is OK
// and that a few guard bytes before and after the buffer were not overwritten.
...
int main() {
  ...
  test_code(
      0x6600 | 0x0074,
      "MBEDTLS_ERR_SSL_INVALID_VERIFY_HASH+MBEDTLS_ERR_SHA256_BAD_INPUT_DATA");
