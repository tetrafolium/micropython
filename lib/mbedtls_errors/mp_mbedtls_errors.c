...
// BEGIN generated code
    ...
    {-(MBEDTLS_ERR_SSL_BAD_HS_SERVER_HELLO_DONE),
     "SSL_BAD_HS_SERVER_HELLO_DONE"},
    {-(MBEDTLS_ERR_SSL_BAD_HS_CLIENT_KEY_EXCHANGE),
     "SSL_BAD_HS_CLIENT_KEY_EXCHANGE"},
    {-(MBEDTLS_ERR_SSL_BAD_HS_CLIENT_KEY_EXCHANGE_RP),
     "SSL_BAD_HS_CLIENT_KEY_EXCHANGE_RP"},
    {-(MBEDTLS_ERR_SSL_BAD_HS_CLIENT_KEY_EXCHANGE_CS),
     "SSL_BAD_HS_CLIENT_KEY_EXCHANGE_CS"},
    {-(MBEDTLS_ERR_SSL_BAD_HS_CERTIFICATE_VERIFY),
     "SSL_BAD_HS_CERTIFICATE_VERIFY"},
    {-(MBEDTLS_ERR_SSL_BAD_HS_CHANGE_CIPHER_SPEC),
     "SSL_BAD_HS_CHANGE_CIPHER_SPEC"},
    ...
    {-(MBEDTLS_ERR_SSL_WAITING_SERVER_HELLO_RENEGO),
     "SSL_WAITING_SERVER_HELLO_RENEGO"},
...
// BEGIN generated code
    ...
    {-(MBEDTLS_ERR_CTR_DRBG_ENTROPY_SOURCE_FAILED),
     "CTR_DRBG_ENTROPY_SOURCE_FAILED"},
    ...
    {-(MBEDTLS_ERR_HMAC_DRBG_ENTROPY_SOURCE_FAILED),
     "HMAC_DRBG_ENTROPY_SOURCE_FAILED"},
...
static size_t mbedtls_err_to_str(int err, const struct ssl_errs tab[],
                                 int tab_len, char *buf, size_t buflen) {
  if (buflen == 0)
    return 0;
  ...
  for (int i = 0; i < tab_len; i++) {
    if (tab[i].errnum == err) {
      strncpy(buf + MBEDTLS_ERR_PREFIX_LEN, tab[i].errstr,
              buflen - MBEDTLS_ERR_PREFIX_LEN);
      buf[buflen - 1] = 0;
      return strlen(buf);
    }
  }

  mbedtls_snprintf(buf + MBEDTLS_ERR_PREFIX_LEN,
                   buflen - MBEDTLS_ERR_PREFIX_LEN, "UNKNOWN (0x%04X)", err);
  return strlen(buf);
}
...
void mbedtls_strerror(int ret, char *buf, size_t buflen) {
  ...
  // If high level code is present, make a concatenation between both error
  // strings.
  if (got_hl) {
    if (buflen < 2)
      return;
